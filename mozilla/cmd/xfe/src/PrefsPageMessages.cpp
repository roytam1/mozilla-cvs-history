...
{
    XtFree((char *)m_autoquote_strings[AUTOQUOTE_PREF_ABOVE]);
    XtFree((char *)m_autoquote_strings[AUTOQUOTE_PREF_BELOW]);
    XtFree((char *)m_autoquote_strings[AUTOQUOTE_PREF_SELECT]);

    XtFree((char *)m_forward_strings[FORWARD_PREF_QUOTED]);
    XtFree((char *)m_forward_strings[FORWARD_PREF_INLINE]);
    XtFree((char *)m_forward_strings[FORWARD_PREF_ATTACH]);
}
...
void XFE_PrefsPageMessages::save()
{
    ...
    // should we autoquote?
    boolval=XmToggleButtonGetState(m_autoquote_toggle);
    ...
    boolval=XmToggleButtonGetState(m_spellcheck_toggle);

    // should we wrap?
    boolval=XmToggleButtonGetState(m_wrap_toggle);
    PREF_SetBoolPref(WRAP_LONG_LINES, boolval);

    // where to wrap?
    charval=XmTextFieldGetString(m_wrap_length_text);
    intval=XP_ATOI(charval);
    XtFree(charval);
    PREF_SetIntPref(WRAP_LENGTH, intval);
    
    // MIME or as-is
    boolval=XmToggleButtonGetState(m_eightbit_quoted_toggle);
