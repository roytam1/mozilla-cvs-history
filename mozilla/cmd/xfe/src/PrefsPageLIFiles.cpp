...
XFE_PrefsPageLIFiles::create()
{
	...
	ac = 0;
	m_cert_toggle = XmCreateToggleButtonGadget(row_column, "certToggle", av, ac);
	XtManageChild(m_cert_toggle);
...
XFE_PrefsPageLIFiles::init()
{
	...
	PREF_GetBoolPref("li.client.security", &enabled);
	XtVaSetValues(m_cert_toggle, XmNset, enabled,
				  XmNsensitive, !PREF_PrefIsLocked("li.client.security"), NULL);
}
...
XFE_PrefsPageLIFiles::save()
{
	...
	PREF_SetBoolPref("li.client.security", 
					XmToggleButtonGetState(m_cert_toggle));
}
