...
then
	...
	then
		MOZ_DIST_BIN=`pwd`
		NSPR_DIST_BIN=`pwd`/../../nsprpub/dist/bin
	fi
...
then
	LD_LIBRARY_PATH=${MOZ_DIST_BIN}:${NSPR_DIST_BIN}${LD_LIBRARY_PATH+":$LD_LIBRARY_PATH"}
else
	LD_LIBRARY_PATH=${MOZ_DIST_BIN}:${NSPR_DIST_BIN}
fi
#
