...
MOZ_SHUNT_PPC2002_API int mkdir(const char* inDirname)
{
    ...
    if(NULL != inDirname)
    {
        TCHAR wDirname[MAX_PATH];

        if(0 != a2w_buffer(inDirname, -1, wDirname, sizeof(wDirname) / sizeof(TCHAR)))
        {
            if(FALSE != CreateDirectory(wDirname, NULL))
            {
                retval = 0;
            }
        }
...
MOZ_SHUNT_PPC2002_API int rmdir(const char* inDirname)
{
    ...
    if(NULL != inDirname)
    {
        ...
        if(0 != a2w_buffer(inDirname, -1, wDirname, sizeof(wDirname) / sizeof(TCHAR)))
        {
            if(FALSE != RemoveDirectory(wDirname))
            {
                retval = 0;
            }
        }
