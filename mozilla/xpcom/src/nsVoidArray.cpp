...
#include <memory.h> // XXX for memcpy()
#include <string.h> // XXX for memmove()
...
nsVoidArray& nsVoidArray::operator=(const nsVoidArray& other)
{
    ...
    if (otherCount != 0) {
        ...
        memcpy(mArray, other.mArray, otherCount * sizeof(void*));
    } else {
        mArray = NULL;
    }
    return *this;
...
PRBool nsVoidArray::InsertElementAt(void* aElement, PRInt32 aIndex)
{
	...
	if (mArray != NULL && aIndex != 0)
            memcpy(newArray, mArray, aIndex * sizeof(void*));
        PRInt32 slide = oldCount - aIndex;
        if (0 != slide) {
            // Slide data over to make room for the insertion
            memcpy(newArray + aIndex + 1, mArray + aIndex,
                      slide * sizeof(void*));
        }
	if (mArray != NULL)
            delete mArray;
...
void nsVoidArray::Compact()
{
    ...
    if (mArraySize != count) {
        ...
        memcpy(newArray, mArray, count * sizeof(void*));
        delete mArray;
        mArray = newArray;
        mArraySize = count;
    }
