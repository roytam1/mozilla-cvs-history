...
NS_IMETHODIMP nsHTMLEditor::InsertHTML(const nsAString & aInString)
{
  return InsertHTMLWithCharsetAndContext(aInString, nsString(), nsString(),
                                         nsString(), nsString(),
                                         nsnull, nsnull, 0, PR_TRUE, PR_FALSE);
}
                                              ...
                                              PRBool aRemoveExistingStyle)
{
  ...
  res = CreateListOfNodesToPaste(fragmentAsNode, address_of(nodeList),
...
nsHTMLEditor::AddInsertionListener(nsIContentFilter *aListener)
{
  ...
  if (!mContentFilters)
  {
    nsresult rv = NS_NewISupportsArray(getter_AddRefs(mContentFilters));
    if (NS_FAILED(rv))
      return rv;
    if (!mContentFilters)
  ...
  PRInt32 foundIndex;
  if (NS_SUCCEEDED(mContentFilters->GetIndexOf(aListener, &foundIndex))
      && foundIndex == -1)
  {
    if (NS_FAILED(mContentFilters->AppendElement(aListener)))
...
nsHTMLEditor::RemoveInsertionListener(nsIContentFilter *aListener)
{
  ...
  if (NS_FAILED(mContentFilters->RemoveElement(aListener)))
    return NS_ERROR_FAILURE;
                                      ...
                                      PRBool *aDoContinue)
{
  *aDoContinue = PR_TRUE;
  ...
  PRUint32 i, numListeners = 0;
  mContentFilters->Count(&numListeners);
  for (i=0; i < numListeners && *aDoContinue; i++)
  {
