...
using namespace esc::v1::CharacterClasses;
namespace esc {
namespace v1 {
namespace lexer {
...
const int slashdiv_context_flag    = 0x1;
const int slashregexp_context_flag = 0x2;
const int lexical_error = 0;
const int lexical_lineterminatorinsinglequotedstringliteral_error = lexical_error+1;
const int lexical_lineterminatorindoublequotedstringliteral_error = lexical_lineterminatorinsinglequotedstringliteral_error+1;
const int lexical_backslashinsinglequotedstring_error = lexical_lineterminatorindoublequotedstringliteral_error +1;
const int lexical_backslashindoublequotedstring_error = lexical_backslashinsinglequotedstring_error +1;
const int lexical_endofstreaminstringliteral_error = lexical_backslashindoublequotedstring_error +1;
const int lexical_nondecimalidentifiercharindecmial_error = lexical_endofstreaminstringliteral_error +1;
const int syntax_error = lexical_nondecimalidentifiercharindecmial_error+1;
...
class Scanner {
	...
	std::vector<Token*>* tokens;   // vector of token instances.
    int slash_context;  // slashdiv_context or slashregexp_context
	std::string codebase;
    bool foundNewlineInComment;
    bool isFirstTokenOnLine;
	int errorCount;
	std::ostream& err; 
...
public:
	...
	Scanner() : err(err), tokens(new std::vector<Token*>()), errorCount(0) {
        slash_context = slashregexp_context_flag;
    }

    Scanner( std::istream& in, std::ostream& err, std::string& codebase )
	    : err(err), tokens(new std::vector<Token*>()), errorCount(0) {
        slash_context = slashregexp_context_flag;
        this->input = new InputBuffer(in, err, codebase);
    ...
    void enterSlashDivContext() {
        slash_context |= slashdiv_context_flag;
    }

    void exitSlashDivContext() {
        slash_context ^= slashdiv_context_flag;
    }

    void enterSlashRexexpContext() {
        slash_context |= slashregexp_context_flag;
    }

    void exitSlashRegexpContext() {
        slash_context ^= slashregexp_context_flag;
    }

    int isSlashDivContext() {
        return slash_context & slashdiv_context_flag;
    }

    int isSlashRegexpContext() {
    ...
    int makeTokenInstance( int token_class, std::string* lexeme ) {
        tokens->push_back( new Token( token_class, lexeme ) );
    ...
    int getTokenClass( int token_id ) {
        ...
        return ((Token*)tokens->at( token_id ))->getTokenClass();
    }
	...
	std::string getTokenText( int token_id ) {
        ...
        // otherwise, get instance data from the instance vector.
        return ((Token*)tokens->at( token_id ))->getTokenText();
    }
	...
	std::string* getPersistentTokenText( int token_id ) {
	...
	const std::string& getTokenSource( int token_id ) {
        ...
        // otherwise, get instance data from the instance vector.
        return ((Token*)tokens->at( token_id ))->getTokenSource();
    }
	...
	std::string getLinePointer() {
        return getLinePointer(std::string(""));
    }

	std::string getLinePointer(std::string& header) {
        std::string padding;
        int i;
		for ( i = header.length() ; i > 0 ; i-- ) {
            padding.append(" ");
        }
        for( i=1 ; i < input->colPos ; i++ ) {
            padding.append(" ");
        }
		padding.append("^");
        return padding;
    ...
    std::string getLinePointer(int pos) {
        std::string padding;
        for( int i=0 ; i < pos ; i++ ) {
            padding.append(" ");
        }
    ...
    void error(int kind, std::string* msg, int tokenid) {
        ...
        switch(kind) {
            ...
            case syntax_error:
            default:
                //loc = ((codebase==0||codebase->length()==0)?"":*codebase+": ") + "Ln " + (input->markLn+1) + ", Col " + (input->markCol+1) + ": ";
                //msg = msg?error_messages[kind]:msg;
                break;
        }
        //printf("%s%s",loc.c_str(),msg->c_str());
        printf("%s\n",input->getLineText(input->positionOfMark()).c_str());
        printf("%s\n",getLinePointer(input->markCol-1).c_str());
        skiperror(kind);
    }
  ...
  **/
  ...
  bool followsLineTerminator() {
      if( debug ) {
          printf("isFirstTokenOnLine = %d",isFirstTokenOnLine);
      }
  ...
  **/
  ...
  int nexttoken() {
    ...
    while ( true ) {
        if( debug ) { 
            printf( "\nstate = %d, next = %d",state,input->positionOfNext());
        }
        ...
        **/
        ...
        case eval_state:
          switch ( nextchar() ) {
            ...
            case '6': case '7': case '8': case '9':
                      state = A_state; continue;
            default: retract(); return makeTokenInstance(identifier_token,input->copy());
          }
   ...
   */
	...
	static void test_nexttoken() {
		...
		Debugger::trace("nexttoken test begin");
		...
		int max_count = sizeof(input)/sizeof(std::string);
		std::ofstream err("test.err");
		for( int i = 0; i < max_count; i++ ) {
			std::istringstream in = std::istringstream(input[i]);
			scanner = new Scanner(in,err,origin);
			result  = scanner->nexttoken();
			if( expected[i] == scanner->getTokenClass(result) )
				printf( "  %d passed %s --> %s : %s\n",i,input[i].c_str(),
				Token::getTokenClassName(scanner->getTokenClass(result)).c_str(), 
						scanner->getTokenText(result).c_str() );
			else {
    ...
    //static void setOut(String filename) throws Exception {
