...
{
    strYarn->mYarn_Buf = ToNewCString(aStr);
    strYarn->mYarn_Size = PL_strlen((const char *) strYarn->mYarn_Buf) + 1;
    strYarn->mYarn_Fill = strYarn->mYarn_Size - 1;
    strYarn->mYarn_Form = 0;     
}
...
nsresult nsAddrDatabase::AddStringColumn(nsIMdbRow* cardRow, mdb_column aInColumn, const nsAString &aStr)
{
    struct mdbYarn yarn;

    GetStringYarn(aStr, &yarn);
    mdb_err err = cardRow->AddColumn(GetEnv(),  aInColumn, &yarn);
