...
NS_IMETHODIMP nsCopyMessageStreamListener::OnStartBinding(nsISupports *ctxt)
{
	nsCOMPtr<nsIMessage> message;
	nsresult rv = NS_OK;
...
NS_IMETHODIMP nsCopyMessageStreamListener::OnStopBinding(nsISupports *ctxt, nsresult aStatus, const PRUnichar *aMsg)
{
	nsresult rv = NS_OK;
	nsCOMPtr<nsIURI> uri = do_QueryInterface(ctxt, &rv);
