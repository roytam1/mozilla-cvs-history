...
{
  dump('In fillFolderPaneContextMenu\n');
  ...
  if (startRange.value < 0)
    return;
  var numSelected = endRange.value - startRange.value + 1;
  var folderResource = GetFolderResource(startRange.value);

  var isServer = GetFolderAttribute(folderResource, 'IsServer') == 'true';
...
function SetupRenameMenuItem(folderResource, numSelected, isServer, serverType, specialFolder)
{
  ...
  var canRename = (GetFolderAttribute(folderResource, 'CanRename') == "true");
...
{
    var canCompact = (GetFolderAttribute(folderResource, 'CanCompact') == "true");
...
{
  var canCreateNew = GetFolderAttribute(folderResource, 'CanCreateSubfolders') == 'true';
