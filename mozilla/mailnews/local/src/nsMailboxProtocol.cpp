...
NS_IMETHODIMP nsMailboxProtocol::OnStartBinding(nsISupports *ctxt)
{
	...
	if (m_nextState == MAILBOX_READ_FOLDER && m_mailboxParser)
	{
		...
		m_mailboxParser->OnStartBinding(ctxt);

	}
	else if(m_mailboxCopyHandler) 
		m_mailboxCopyHandler->OnStartBinding(ctxt); 
...
NS_IMETHODIMP nsMailboxProtocol::OnStopBinding(nsISupports *ctxt, nsresult aStatus, const PRUnichar *aMsg)
{
	if (m_nextState == MAILBOX_READ_FOLDER && m_mailboxParser)
	{
		// we need to inform our mailbox parser that there is no more incoming data...
		m_mailboxParser->OnStopBinding(ctxt, 0, nsnull);
	}
	else if (m_mailboxCopyHandler) 
		m_mailboxCopyHandler->OnStopBinding(ctxt, 0, nsnull); 
