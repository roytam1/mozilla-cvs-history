...
nsresult nsAbSync::LoadListHistoryTable()
{
  nsresult rv = NS_OK;
  PRUint32 fileSize;
  ...
  if (NS_SUCCEEDED(rv) && members)
  {
    ...
    if (total)
    {
      ...
      for (i = 0; i < total; i++)
      {
        ...
        nsCOMPtr<nsIAbMDBCard> pCard = do_QueryInterface(item, &rv);
        if (NS_FAILED(rv) || !pCard)
          break;

        PRUint32 aKey;
        if (NS_FAILED(pCard->GetKey(&aKey)) || (aKey <=0))
          break;

        // This member may already has the server id so check it.
        PRInt32 serverID;
        LocateServerIDFromClientID(aKey, &serverID);

        mListNewSyncMapingTable[listIndex].memLocalID.Add(aKey);
        mListNewSyncMapingTable[listIndex].memServerID.Add(serverID);
        mListNewSyncMapingTable[listIndex].memFlags.Add(SYNC_ADD);  // may not nee this

        AddAListMememerToProtocolLine(listKey, aKey, i, protLine);
      }
    }
    // If fails for any reason, rest protocol string so we don't send something wront to the server.
    if (NS_FAILED(rv))
      protLine.Truncate();
  }
  return rv;
...
void nsAbSync::AddAListMememerToProtocolLine(PRUint32 listKey, PRUint32 memberKey, PRUint32 cid, nsString &protLine)
{
  ...
  protLine.Append(NS_ConvertASCIItoUCS2(SYNC_ESCAPE_ADD_LIST_CONTACT));
  mCurrentPostRecord++;
  PR_FREEIF(tVal);
...
// For a given list card, generate protocol string for all changed members.
nsresult nsAbSync::GenerateMemberProtocolForChangedList(nsIAbCard *listCard, PRUint32 listIndex, nsString &protLine)
{
  nsresult rv;
  nsCOMPtr<nsISupportsArray> members;
  ...
  if (NS_SUCCEEDED(rv) && members)
  {
    ...
    rv = LocateListHistoryRecord(listKey, &historyRecord);
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    // Now, check members that have been deleted.
...
void nsAbSync::CheckDeletedMembers(syncListMappingRecord *listRecord, nsString &protLine)
{
  ...
  for (PRUint32 i = 0; i < cnt; i++)
    ...
    {
      // ok, this member has been removed.
      if (!protLine.IsEmpty())
          protLine.Append(NS_LITERAL_STRING("&"));
      char *tVal = PR_smprintf("%d", mCurrentPostRecord);
      protLine.Append(NS_ConvertASCIItoUCS2(tVal) + NS_LITERAL_STRING("="));
      protLine.Append(NS_ConvertASCIItoUCS2(SYNC_ESCAPE_DEL_LIST_CONTACT));
      mCurrentPostRecord++;
      PR_FREEIF(tVal);

      tVal = PR_smprintf("%d", listRecord->serverID);
      if (tVal)
      {
        protLine.Append(NS_LITERAL_STRING("%26list_id%3D") + NS_ConvertASCIItoUCS2(tVal));
        nsCRT::free(tVal);
...
nsresult nsAbSync::LocateListRecord(PRUint32 listID, syncListMappingRecord **result)
{
...
nsresult nsAbSync::LocateListHistoryRecord(PRUint32 listID, syncListMappingRecord **result)
{
  NS_ENSURE_ARG_POINTER(result);
...
nsresult nsAbSync::AddNewMailingListMembers()
{
  ...
  for (i = 0; i < addCount; i++)
  {
    ...
    if (NS_FAILED(LocateListRecord(listLocalID, &listRecord)))
      continue;
    ...
    // Now, now the member to the list.
...
nsresult nsAbSync::DeleteMailingListMembers()
{
  ...
  for (i = 0; i < delCount; i++)
  {
    ...
    if (NS_SUCCEEDED(rv))
    {
      ...
      rv = LocateListRecord(listLocalID, &listRecord);
      if (NS_SUCCEEDED(rv))
      {
        PRUint32 cnt = listRecord->memLocalID.GetSize();
...
nsresult nsAbSync::DeleteMailingListEmailMembers()
{
  return NS_OK;
}
...
nsresult nsAbSync::AddNewGroups()
{
  ...
  {    
    ...
    // Set default email to "1" (hardcode it here).
    nsString defaulEmail;
    defaulEmail.AssignWithConversion("1");
    ...
    // Now, calculate the NEW CRC for the new group (for now use the same code as list. we may
    // need a new func to check group_url changes which is mapped to home web page field).
    if (NS_FAILED(GenerateProtocolForList(newCard, PR_FALSE, tempProtocolLine)))
      continue;
