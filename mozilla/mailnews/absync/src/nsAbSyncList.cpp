...
nsresult nsAbSync::LoadListsFromHistoryFile()
{
  ...
  nsresult rv = NS_OK;
  PRUint32 fileSize;
  rv = mListHistoryFile->GetFileSize(&fileSize);
  NS_ENSURE_SUCCESS(rv,rv);

  rv = mListHistoryFile->OpenStreamForReading();
  NS_ENSURE_SUCCESS(rv,rv);
...
{
  if (!crc || !localId || !serverId)
    return;

  *crc = *localId = *serverId = 0;
  if (!str)
    return;
...
nsresult nsAbSync::InitListSyncTable(nsIEnumerator *cardEnum)
{
  ...
  if (! mListNewTableCount)
    return NS_OK;

  mListNewSyncMapingTable = (syncListMappingRecord *) PR_MALLOC(mListNewTableCount * sizeof(syncListMappingRecord));
  if (!mListNewSyncMapingTable)
    return NS_ERROR_OUT_OF_MEMORY;

  // Init the memory!
  memset(mListNewSyncMapingTable, 0, (mListNewTableCount * sizeof(syncListMappingRecord)));
  return NS_OK;
}
...
nsresult nsAbSync::SaveCurrentListsToHistoryFile()
{
  ...
  nsresult rv = mListHistoryFile->OpenStreamForWriting();
  NS_ENSURE_SUCCESS(rv,rv);

  // Ok, these are the lists that exist when we started the sync op.
  PRInt32 writeSize;
  PRUint32 writeCount = 0;
  while (writeCount < mListNewTableCount)
  {
    ...
    if (mListNewSyncMapingTable[writeCount].serverID != 0)
    {
      ...
      rv = mListHistoryFile->Write(str.get(),  str.Length(), &writeSize);
      if (NS_FAILED(rv) || (writeSize != (PRInt32)str.Length()))
      {
        mListHistoryFile->CloseStream();
        return rv;
      }
...
{
  NS_ENSURE_ARG_POINTER(listCard);
  ...
  if (NS_SUCCEEDED(rv) && members)
  {
    ...
    if (total)
    {
      ...
      if (! emailString.IsEmpty())
      {
        ...
        mListNewSyncMapingTable[listIndex].CRC = GetCRC((char *)NS_ConvertUCS2toUTF8(emailString).get());
      }
    }
...
void nsAbSync::AppendProtocolCmdHeader(nsString &protLine, const char *cmd)
{
  ...
  protLine.AppendInt(mCurrentPostRecord);
  protLine.Append(NS_LITERAL_STRING("="));
...
nsresult nsAbSync::CheckCurrentListForChangedMember(nsIAbCard *listCard, PRUint32 listIndex, nsString &protLine)
{
  ...
  if (NS_SUCCEEDED(rv) && members)
  {
    ...
    ulong newCRC = 0;
    if (! emailString.IsEmpty())
      newCRC = GetCRC((char *)NS_ConvertUCS2toUTF8(emailString).get());
...
nsresult nsAbSync::LocateExistingListRecord(PRUint32 listID, syncListMappingRecord **result)
{
  ...
  {
    PRUint32 writeCount = 0;
    PRUint32 cnt = (PRUint32) mListNewServerTable->Count();
...
nsresult nsAbSync::LocateHistoryListRecord(PRUint32 listID, syncListMappingRecord **result)
{
  ...
  for (PRUint32 i = 0; i < mListOldTableCount; i++)
  {
...
nsresult nsAbSync::AddNewMailingLists()
{
  ...
  {    
    ...
    localID = HuntForExistingABEntryInServerRecord(i, aDatabase, directory, PR_FALSE, &serverID, getter_AddRefs(tCard));
...
nsresult nsAbSync::DeleteMemberFromList(nsIAbCard *listCard, PRInt32 memberLocalID)
{
  NS_ENSURE_ARG_POINTER(listCard);
...
nsresult nsAbSync::DeleteMailingListMembers()
{
  ...
  for (i = 0; i < delCount; i++)
  {
    ...
    rv = DeleteMemberFromList(listCard, memberLocalID);
  ...
  {    
    ...
    for (j = 0; j < cnt; j++)
      if ( (GetCardTypeByMemberId(listRecord->memLocalID.GetAt(j)) & SYNC_IS_AOL_ADDITIONAL_EMAIL) )
      {
        ...
        rv = DeleteMemberFromList(listCard, listRecord->memLocalID.GetAt(j));
        if (NS_SUCCEEDED(rv))
        {
    ...
    if (emailString.IsEmpty())
      ...
      listRecord->CRC = GetCRC((char *)NS_ConvertUCS2toUTF8(emailString).get());
...
{
  NS_ENSURE_ARG_POINTER(listRecord);
...
{
  NS_ENSURE_ARG_POINTER(idString);
...
{
  NS_ENSURE_ARG_POINTER(addrString);
