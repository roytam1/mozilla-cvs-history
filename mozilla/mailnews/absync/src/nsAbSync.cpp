...
nsAbSync::AnalyzeAllRecords(nsIAddrDatabase *aDatabase, nsIAbDirectory *directory, PRBool analyzeUser)
{
  ...
  //
  CheckDeletedRecords(aDatabase, directory);
...
void nsAbSync::ParseDateFields(nsIAbCard *aCard, nsString *aTagName, nsString *aTagValue)
{
  // Parse out date (in aTagValue) like 03/05/1996 and set yy, mm and dd accordingly.
  PRInt32 atPos, cnt = 0;
  nsString tempStr, digits;
...
void nsAbSync::CheckDeletedRecords(nsIAddrDatabase  *aDatabase, nsIAbDirectory *directory)
{
  ...
  printf("ABSYNC: DELETING Card: %d\n", mOldSyncMapingTable[readCount].serverID);
        ...
        if (tVal2)
          mPostString.Append(NS_ConvertASCIItoUCS2(tVal2) + NS_LITERAL_STRING("="));
        ...
        if (mOldSyncMapingTable[readCount].flags & SYNC_IS_AOL_GROUPS)
          mPostString.Append(NS_ConvertASCIItoUCS2(SYNC_ESCAPE_DELGROUP) +
                             NS_LITERAL_STRING("%26id=") + NS_ConvertASCIItoUCS2(tVal));
        ...
        { // All members will be deleted once the list is removed.
          mPostString.Append(NS_ConvertASCIItoUCS2(SYNC_ESCAPE_DELLIST) +
                             NS_LITERAL_STRING("%26list_id=") + NS_ConvertASCIItoUCS2(tVal));
        }
        nsCRT::free(tVal);
        nsCRT::free(tVal2);
        mCurrentPostRecord++;
