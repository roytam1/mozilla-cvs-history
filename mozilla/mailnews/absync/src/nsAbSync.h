...
private:
  ...
  PRUint32                        mOldTableCount;         // Card count in the old history table.
  syncMappingRecord               *mOldSyncMapingTable;   // Old history table.
  PRUint32                        mNewTableCount;         // Card count int the existing address book.
  syncMappingRecord               *mNewSyncMapingTable;   // New table for the existing address book.
  nsVoidArray                     *mNewServerTable;       // New entries from the server.
// For lists
  PRUint32                        mListOldTableCount;      // List count in the old list history table.
  syncListMappingRecord           *mListOldSyncMapingTable;// Old history table for list.
  PRUint32                        mListNewTableCount;      // List count int the existing address book.
  ...
  nsresult        RecoverUserSyncRecords(PRUint32 numOfCards, nsIEnumerator *cardEnum);
  nsresult        LoadUsersFromHistoryFile();
  nsresult        SaveCurrentUsersToHistoryFile();
  void            CheckDeletedRecords();
  nsresult        InitUserSyncTable(PRUint32 numOfCards, nsIEnumerator *cardEnum);
  ...
  nsresult        ParseDateFields(nsIAbCard *aCard, nsString *aTagName, nsString *aTagValue);
  ...
  nsresult        DeleteMemberFromList(nsIAbCard *listCard, PRInt32 memberLocalID);
