...
NS_IMPL_LOG(NNTPLog)
#define PRINTF(args) NS_LOG_PRINTF(NNTPLog, args)
#define FLUSH()      NS_LOG_FLUSH(NNTPLog)
...
nsNNTPProtocol::InitializeNewsFolderFromUri(const char *uri)
{
		...
		PRINTF(("InitializeNewsFolderFromUri(%s)",uri));
...
PRInt32 nsNNTPProtocol::BeginAuthorization()
{
	...
	if (cachedUsername) {
		PRINTF(("use %s as the username", (const char *)cachedUsername));
		NET_SACat(&command, (const char *)cachedUsername);
	}
	else {
		PRINTF(("use %s as the username", (const char *)username));
...
PRInt32 nsNNTPProtocol::AuthorizationResponse()
{
    ...
    if (MK_NNTP_RESPONSE_AUTHINFO_OK == m_responseCode ||
	...
	else if (MK_NNTP_RESPONSE_AUTHINFO_CONT == m_responseCode)
	  {
		...
		if (cachedPassword) {
			PRINTF(("use %s as the password", (const char *)cachedPassword));			NET_SACat(&command, (const char *)cachedPassword);
		}
		else {
...
PRInt32 nsNNTPProtocol::DisplayNewsgroups()
{
	...
	PRINTF(("about to display newsgroups. path: %s", m_path));
    ...
    if (line[0]=='.' && line[1]=='\0')
	{
		...
		if (NS_SUCCEEDED(rv) && xactive)
		{
		  ...
		  if (NS_SUCCEEDED(rv) && m_newsgroup)
		  {
				...
				PRINTF(("listing xactive for %s", groupName));
				PR_FREEIF(line);
				return 0;
		  }
		}
...
PRInt32 nsNNTPProtocol::FigureNextChunk()
{
	...
	if (m_firstArticle > 0) 
	{
      ...
      PRINTF(("add to known articles:  %d - %d",  m_firstArticle, m_lastArticle));
	...
	PRINTF(("Chunk will be (%d-%d)",  m_firstArticle, m_lastArticle));
...
PRInt32 nsNNTPProtocol::XoverSend()
{
	...
	PRINTF(("XOVER %d-%d", m_firstArticle, m_lastArticle));
...
PRInt32 nsNNTPProtocol::ReadNewsgroupBody(nsIInputStream * inputStream, PRUint32 length)
{
  ...
  PRINTF(("read_group_body: got line: %s|", line));
...
/*		m_nextState = NEWS_DONE; */
	...
	if (line)
	{
		if (line[0] != '.')
		{
			...
			PRINTF(("adding pretty name %s", prettyName));
		}
		else
		{
		...
		if (line[0] != '.')
		{
			...
			if (s)
			{
				...
				*s = 0;
				...
				PRINTF(("got xactive for %s of %s", line, flags));
