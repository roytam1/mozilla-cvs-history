...
net_FileLoad (ActiveEntry * cur_entry)
{
	...
	connection_data = (FILEConData*) cur_entry->con_data;
...
#if defined(SMOOTH_PROGRESS)
    if (cur_entry->window_id && cur_entry->window_id->progressManager) {
        nsFileTransfer* transfer = new nsFileTransfer(cur_entry);
        if (transfer) {
            connection_data->transfer = transfer;
            transfer->AddRef();
...
#if defined(SMOOTH_PROGRESS)
                if (connection_data->transfer) {
                    connection_data->transfer->Release();
                    connection_data->transfer = NULL;
                }
...
{
	index_format_conv_data_object *obj = (index_format_conv_data_object*) stream->data_object;	
...
{
	index_format_conv_data_object *obj = (index_format_conv_data_object*) inputStream->data_object;
