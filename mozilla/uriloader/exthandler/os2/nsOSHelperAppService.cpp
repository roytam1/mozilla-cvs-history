...
NS_IMETHODIMP nsOSHelperAppService::LoadUrl(nsIURI * aURL)
{
  ...
  rv = NS_NewLocalFile(NS_ConvertUTF8toUCS2(applicationName), PR_FALSE, getter_AddRefs(application));
  if (NS_FAILED(rv)) {
     /* Maybe they didn't qualify the name - use CMD.EXE */
//     rv = NS_NewLocalFile(getenv("COMSPEC"), PR_FALSE, getter_AddRefs(application));
     rv = NS_NewLocalFile(NS_LITERAL_STRING("C:\\OS2\\CMD.EXE"), PR_FALSE, getter_AddRefs(application));
     if (NS_FAILED(rv)) {
       return rv;
     }
