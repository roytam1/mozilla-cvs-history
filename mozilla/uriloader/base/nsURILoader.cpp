...
protected:
    ...
    nsresult InvokeUnknownContentHandler(nsIRequest *request, const char * aContentType, nsIDOMWindowInternal * aDomWindow);
  ...
  if (NS_SUCCEEDED(rv))
  {
    ...
    if (contentListener)
    {
      ...
      if (NS_FAILED(rv))
      {
        ...
        return InvokeUnknownContentHandler(request, contentType, domWindow);
      }
...
nsresult nsDocumentOpenInfo::InvokeUnknownContentHandler(nsIRequest *request, const char * aContentType, nsIDOMWindowInternal * aDomWindow)
{
  ...
  return handler->HandleUnknownContentType( request, aContentType, aDomWindow );
}
