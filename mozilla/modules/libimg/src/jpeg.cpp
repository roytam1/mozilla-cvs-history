...
/*
	...
	icc profiles from/into the jpeg data stream.
	The read case is a 'push', and we'll use it to tell the image container
	that there is a profile along with the image.  The write case doesn't
	really occur in the borwser context, except when composing html.
	Since this is outside the scope of this project, I'm leaving the write
	routine as an internal function for now.  It can be promoted tpo external
	and put into a public header at that point.
*/
static void
setup_read_icc_profile	JPP((j_decompress_ptr		cinfo));

static PRBool
read_icc_profile		JPP((j_decompress_ptr		cinfo,
				     		JOCTET					**icc_data_ptr,
				     		unsigned int			*icc_data_len));
				     				
static void
write_icc_profile		JPP((j_compress_ptr			cinfo,
							const JOCTET			*icc_data_ptr,
							unsigned int			icc_data_len));
static PRBool
marker_is_icc			JPP((jpeg_saved_marker_ptr	marker));

/*
 * More stuff for the icc_profile read/write implementation.
 * -------------------------------------------------------------------------
		...
		case JPEG_HEADER:
			if (jpeg_read_header(jd, TRUE) != JPEG_SUSPENDED) {
				...
				/*
					...
					container about it.  We must do this before
					the color space converter is set up.
				*/
				if (il_color_matching_available())
...
#define MAX_SEQ_NO  255		/* sufficient since marker numbers are bytes */
  ...
  *icc_data_len = 0;
  ...
  icc_data = (JOCTET *) PR_MALLOC(total_length * sizeof(JOCTET));
  if (icc_data == NULL)
