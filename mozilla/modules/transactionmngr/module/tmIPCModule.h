...
/**
  ...
  *   layer the transaction service uses. As it is, I think some dependancies
  *   on the IPC daemon impl have creeped into the tmTransactionManager and
  *   it's support classes, but they are minor and if need be can surely be
  *   removed.
  */
class tmIPCModule
{
...
protected:
  ...
  /**
    ...
    * @returns NS_ERROR_OUT_OF_MEMORY if creating the TM fails
    * @returns NS_ERROR_FAILURE if something else fails creating the TM
    */
  static nsresult InitInternal();
