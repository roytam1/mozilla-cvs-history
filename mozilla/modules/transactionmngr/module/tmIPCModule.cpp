...
tmIPCModule::HandleMsg(ipcClientHandle client, const nsID &target, 
  ...
  nsresult rv = NS_ERROR_OUT_OF_MEMORY;    // prime it for new
  ...
  // initialize it
  if (trans) {
    rv = trans->Init(IPC_GetClientID(client), (PRUint8 *)data, dataLen);
    if (NS_SUCCEEDED(rv)) {
...
// straight pass-through, don't check args, let the TS do it.
void
