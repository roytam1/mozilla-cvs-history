...
static NS_DEFINE_IID(kILiveConnectPluginIID, NP_ILIVECONNECTPLUGIN_IID); 
static NS_DEFINE_IID(kIPluginIID, NP_IPLUGIN_IID); 
                         ...
                         NPIPlugin* *result)
{
    ...
    if (pfnGetEntryPoints == NULL)
        return NPPluginError_InvalidFunctableError;
    ...
    if (pfnGetEntryPoints(&callbacks) != NPERR_NO_ERROR)
    ...
    if (HIBYTE(callbacks.version) < NP_VERSION_MAJOR)
        return NPPluginError_IncompatibleVersionError;
#endif    
    ...
    if (pfnInitialize == NULL)
        return NPPluginError_ModuleLoadFailedError; // XXX Right error?

    if (pfnInitialize(&ns4xPlugin::CALLBACKS) != NPERR_NO_ERROR)
        return NPPluginError_ModuleLoadFailedError; // XXX shoudl convert the 4.x error...
...
ns4xPlugin::NewInstance(NPIPluginInstancePeer* peer, NPIPluginInstance* *result)
{
    ns4xPluginInstance* inst =
        new ns4xPluginInstance(peer, &fCallbacks);

    if (inst == NULL)
        return NPPluginError_OutOfMemoryError;

    inst->AddRef(); // Increment the reference count to "one"...

    NPPluginError error;
    if ((error = inst->Initialize()) != NPPluginError_NoError) {
        inst->Release();
        return error;
    }
...
ns4xPlugin::GetMIMEDescription(void)
{
    return NULL; // XXX make a callback, etc.
}


NS_IMETHODIMP_(jclass)
ns4xPlugin::GetJavaClass(void)
{
    return fCallbacks.javaClass;
}
...
ns4xPlugin::_geturl(NPP npp, const char* relativeURL, const char* target)
{
    ...
    return (NPError) peer->GetURL(relativeURL, target, NULL, NULL, NULL, FALSE);
}
                          ...
                          void* notifyData)
{
    ...
    return (NPError) peer->GetURL(relativeURL, target, notifyData,
                                  NULL, NULL, FALSE);
}
                           ...
                           void* notifyData)
{
    ...
    return (NPError) peer->PostURL(relativeURL, target, len, buf, file, notifyData,
                                   NULL, FALSE);
}
                     ...
                     const char *buf, NPBool file)
{
    ...
    return (NPError) peer->PostURL(relativeURL, target, len, buf, file,
                                   NULL, NULL, FALSE);
}
