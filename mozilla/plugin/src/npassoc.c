...
np_parse_exts(const char *exts)
{
	...
	for(e=p=(char *)exts; *p; p++)
	{
		if( (*p==' ') || (*p==',') )
		{
			...
			if(p>e) 
			{				
				if(!(np_array[n] = (char *)PR_MALLOC(p-e+1)))
					return 0;
	...
	if(!(res = (char **)PR_MALLOC((n+1)*sizeof(char *))))
		return 0;
...
NPL_NewFileAssociation(const char *MIMEType, const char *extensions, const char *description, void *fileType)
{
    ...
    if(!(fassoc=PR_NEWZAP(NPFileTypeAssoc)))
        return 0;

	fassoc->type         = PL_strdup(MIMEType ? MIMEType : "");
	fassoc->description  = PL_strdup(description ? description : "");
	fassoc->extentstring = PL_strdup(extensions ? extensions : "");
...
NPL_DeleteFileAssociation(NPFileTypeAssoc *fassoc)
{
	...
	if (fassoc->type) 
	{ 
		PR_FREEIF(fassoc->type);
		fassoc->type = NULL;
	}

	if (fassoc->description) 
	{ 
		PR_FREEIF(fassoc->description);
		fassoc->description = NULL;
	}

	if (fassoc->extentstring) 
	{ 
		PR_FREEIF(fassoc->extentstring);
...
NPL_RegisterFileAssociation(NPFileTypeAssoc *fassoc)
{
	if (fassoc)
	{
		...
		if (fassoc->description)
		{
			...
			PR_ASSERT(cdata);
			if (cdata)
                cdata->ci.desc = fassoc->description ?
                    PL_strdup(fassoc->description) : NULL;
		}
	}
}
