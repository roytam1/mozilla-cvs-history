...
int PR_CALLBACK nsUnicodeMappingUtil::PrefChangedCallback(const char* aPrefName, void* instance_data)
{
	...
	// we just clear the old state here. it will get lazily re-initted the next
	// time someone requests the singleton
	nsUnicodeMappingUtil::InternalFetchSingleton()->CleanUp();
	return 0;
}
...
{
  if (mInitialized)     // check to see if we are already initialized
    return;
...
{	
  nsUnicodeMappingUtil* mappingUtil = (nsUnicodeMappingUtil*)aClosure;
  ...
  ScriptCode script = nsUnicodeMappingUtil::MapLangGroupToScriptCode(langGroup.get());
  ...
  nsAutoString genNameString; genNameString.AssignWithConversion(genName.get());
  nsGenericFontNameType type = nsUnicodeMappingUtil::MapGenericFontNameType(genNameString);
  if (type >= kUknownGenericFontName)
  	return;
  	
  char* valueInUTF8 = nsnull;
  mappingUtil->mPref->CopyCharPref(aName, &valueInUTF8);
  if ((nsnull == valueInUTF8) || !*valueInUTF8)
  {
	  nsMemory::Free(valueInUTF8);
  	return;
  }
  
  /*
  // is this code really just doing utf8 to ucs2?
  ...
  */
  ...
  nsString *fontname = new NS_ConvertUTF8toUCS2(valueInUTF8);
  nsMemory::Free(valueInUTF8);
  if (nsnull == fontname || fontname->IsEmpty())
  {
    delete fontname;    // ok with null
  	return;
  }
  
  short fontID = 0;
  if( (! nsFontEnumeratorMac::GetMacFontID(*fontname, fontID)) ||
  ...
  if (mappingUtil->mGenericFontMapping[script][type])
  	delete mappingUtil->mGenericFontMapping[script][type];
  	
  mappingUtil->mGenericFontMapping[script][type] = fontname;
...
/*
				...
				PRUint32	n = nsTextFormatter::snprintf(valueInUCS2, FACESIZE, format, valueInUTF8);
*/
        NS_ConvertUTF8toUCS2    ucs2FontName(valueInUTF8);
				nsMemory::Free(valueInUTF8);
				if (!ucs2FontName.IsEmpty())
					{
						short	fontID = 0;
					if (nsFontEnumeratorMac::GetMacFontID(ucs2FontName, fontID))
...
{
	static const ScriptCode prebuiltMapping[kUnicodeBlockSize] = 
	{
	...
	{
		if (ScriptEnabled(prebuiltMapping[i]))
		{
			mBlockToScriptMapping[i] = prebuiltMapping[i];
		}
		else
	{
...
nsUnicodeMappingUtil* nsUnicodeMappingUtil::GetSingleton()
{
  ...
  gSingleton->Init();   // make sure we're initted, since we init lazily
	return gSingleton;
}


//--------------------------------------------------------------------------
// This method does not ensure that the singletone has been initted. it's used
// from the pref changed callback, avoiding multiple reset/inits.
nsUnicodeMappingUtil* nsUnicodeMappingUtil::InternalFetchSingleton()
{
	if (! gSingleton)
		gSingleton = new nsUnicodeMappingUtil();
...
//--------------------------------------------------------------------------
void nsUnicodeMappingUtil::FreeSingleton()
{
    delete gSingleton;
    gSingleton = nsnull;
}
