...
typedef void (*DownSampleFunc) (struct Image* aImage, uint8* aColorCube,
...
static uint32* gDM565 = NULL;
static uint8* gColorCube666 = NULL;
...
void
DownSampleImage(struct Image* aImage, uint8 aDevice)
{
//    uint8* colorCube = NULL;
//    DownSampleFunc converter = NULL;
//    int nR = 0, nG = 0, nB = 0;

    // Select Appropriate Colour Cube and Down-Sampling function
    /*switch (aDevice) {
    case MEMBUF_DEVICE_BW:
        {
            converter = DownSample1;
        }
        break;
    case MEMBUF_DEVICE_GREY16:
        {
            converter = DownSample4GrayD;
        }
        break;
    case MEMBUF_DEVICE_COLOR16:
        {
            converter = DownSample4D;
        }
        break;
    case MEMBUF_DEVICE_COLOR256:
        {
            nR = 6, nG = 6, nB = 6;
            if (!gColorCube666)
                gColorCube666 = MakeColorCube(nR, nG, nB);

            colorCube = gColorCube666;
            converter = DownSample8D;
        }
        break;
    case MEMBUF_DEVICE_COLOR16b:
        {
            converter = DownSample16D;
        }
        break;
    }

    // Perform the Down-Sample with the appropriate Function & Colour Cube
    if (converter)
        converter(aImage, colorCube, nR, nG, nB);
    */
}


uint8*
MakeDitherCube(unsigned long* aPixels, int aR, int aG, int aB)
{
    int r, g, b;
    int i;
    int idx;

    uint8* colorCube = (uint8*)calloc(512, 1);
    for (i = 0; i < 512; ++i) {
        r = MIN(aR - 1, i >> 6);
        g = MIN(aG - 1, (i >> 3) & 7);
        b = MIN(aB - 1, i & 7);
...
uint8*
MakeColorCube(int aR, int aG, int aB)
{
    int i = 0;
    int r, g, b, rTemp, gTemp, bTemp;
    ...
    for (r = 0; r < aR; ++r) {
        for (g = 0; g < aG; ++g) {
            for (b = 0; b < aB; ++b) {
                ...
                gTemp = g * 65535 / (aG - 1);
                bTemp = b * 65535 / (aB - 1);
                pixels[i++] = NS_RGB(rTemp, gTemp, bTemp);
            }
        }
    }
    return MakeDitherCube(pixels, aR, aG, aB);
}
...
/*
// XXXben - Tiling may or may not work.
//          http://lxr.mozilla.org/seamonkey/source/gfx/src/xlibrgb/xlibrgb.c#1641
//          |xxlib_rgb_convert_8_d|
void
DownSample8D(struct Image* aImage, uint8* aColorCube,
             int aR, int aG, int aB)
{
    uint8* inBuf    = aImage->bytes;
    int inBPL       = aImage->nbytes / aImage->h;

    int outBPL      = (aImage->w * 8) >> 5;
    if ((aImage->w * 8) & 0x1F)
        ++outBPL;
    outBPL <<= 2;
    ...
    int outByteCount = outBPL * aImage->h;
    uint8* outBytes = (uint8*)malloc(sizeof(uint8) * outByteCount);

    uint8* outBuf = outBytes; // + aImage->yoffset * lineStride + aImage->xoffset;
    for (int y = 0; y < aImage->h; ++y) {
        const uint8* dmp = DM[y & (DM_HEIGHT - 1)];
        uint8* ibp = inBuf;
        uint8* obp = outBuf;
        for (int x = 0; x < aImage->w; ++x) {
            int dith = (dmp[x & (DM_WIDTH - 1)] << 2) | 7;

            int r = ((*ibp++ * rs) + dith) >> 8;
            int g = ((*ibp++ * gs) + (262 - dith)) >> 8;
            int b = ((*ibp++ * bs) + dith) >> 8;

            *obp++ = aColorCube[(r << 6) | (g << 3) | b];
        }
        inBuf += inBPL;
        outBuf += outBPL;
    }

    // We don't free the old (24-bit) buffer because it belongs to the
    // |nsIImage| implementing |membufImage|, not us. Simply making the Image
    // structure use the new buffer for encoding is enough.
    aImage->bytes = outBytes;
    aImage->nbytes = outByteCount;
    aImage->bpp = 1;
}

void MakeDitherMap565()
{
    int i;
    uint32 dith;

    if (gDM565 == NULL) {
        gDM565 = (uint32*)malloc(sizeof(uint32) * DM_WIDTH * DM_HEIGHT);
        uint32* dmp = gDM565;
        for (i = 0; i < DM_WIDTH * DM_HEIGHT; ++i) {
            dith = DM[0][i] >> 3;
            *dmp++ = (dith << 20) | dith | (((7 - dith) >> 1) << 10);
        }
    }
}

void
DownSample16D(struct Image* aImage, uint8* aColorCube,
              int aR, int aG, int aB)
{
    int inBPL = aImage->nbytes / aImage->h;
    int outBPL = (aImage->w * 16) >> 5;
    if ((aImage->w * 16) & 0x1f)
        ++outBPL;
    outBPL <<= 2;

    uint8* inBytes = aImage->bytes;
    uint8* outBytes = (uint8*)malloc(outBPL * aImage->h);

    uint8* ibp = aImage->bytes;
    uint8* obp = outBytes; // + aImage->yoffset * lineStride + aImage->xoffset * 2;

    MakeDitherMap565();

    for (int y = 0; y < aImage->h; ++y) {
        uint32* dmp = gDM565 + ((y & (DM_HEIGHT - 1)) << DM_WIDTH_SHIFT);
        unsigned char* bp2 = ibp;
        ...
        for (int x = 0; x < aImage->w; ++x) {
            ...
            ((unsigned short *)obp)[x] = ((rgb & 0x0f800000) >> 13) |
                                         ((rgb & 0x0003e000) >> 8) |
                                         ((rgb & 0x000000f8) >> 3);
        }
        ibp += inBPL;
    ...
    // structure use the new buffer for encoding is enough.
    aImage->bytes = outBytes;
    aImage->nbytes = outBPL * aImage->h;
    aImage->bpp = 2;
}

void
DownSample1(struct Image* aImage, uint8* aColorCube,
            int aR, int aG, int aB)
{
...
void
DownSample4GrayD(struct Image* aImage, uint8* aColorCube,
                 int aR, int aG, int aB)
{
...
void
DownSample4D(struct Image* aImage, uint8* aColorCube,
             int aR, int aG, int aB)
{
