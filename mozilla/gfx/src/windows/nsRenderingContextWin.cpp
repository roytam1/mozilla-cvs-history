...
NS_IMETHODIMP nsRenderingContextWin :: GetWidth(const char* aString,
                                                ...
                                                nscoord& aWidth)
{
  NS_ASSERTION(mFontMetrics, "Null font metrics");
  SetupFontAndColor();

  // Check for the very common case of trying to get the width of a single
  // space.
  if ((1 == aLength) && (aString[0] == ' ')) {
    aWidth = mCurrFontWin->mSpaceWidth;
    return NS_OK;
  }
...
NS_IMETHODIMP
nsRenderingContextWin::GetDimensions(const char*   aString,
                                     PRInt32       aLength,
                                     PRInt32       aAvailWidth,
                                     PRInt32*      aBreaks,
                                     PRInt32       aNumBreaks,
                                     ...
                                     PRInt32*      aFontID)
{
  ...
  if (nsnull != mFontMetrics) {
    ...
    nscoord width = 0;
    PRInt32 start = 0;
    nscoord aveCharWidth = mCurrFontWin->mAveCharWidth;
...
static PRBool PR_CALLBACK
do_BreakGetDimensions(const nsFontSwitch* aFontSwitch,
                      const PRUnichar*    aSubstring,
                      PRUint32            aSubstringLength,
                      void*               aData)
{
  ...
  BreakGetDimensionsData* data = (BreakGetDimensionsData*)aData;
  if (data->mFont != fontWin->mFont) {
    data->mFont = fontWin->mFont;
    ::SelectObject(data->mDC, data->mFont);
  }
  ...
  data->mOffsets->AppendElement((void*)&pstr[start]);
  return PR_TRUE; // don't stop if we can still fit more characters
}

NS_IMETHODIMP
nsRenderingContextWin::GetDimensions(const PRUnichar* aString,
                                     PRInt32          aLength,
                                     PRInt32          aAvailWidth,
                                     PRInt32*         aBreaks,
                                     PRInt32          aNumBreaks,
                                     ...
                                     PRInt32*         aFontID)
{
  ...
  BreakGetDimensionsData data = {mDC, mCurrFont, mP2T, 
    aAvailWidth, aBreaks, aNumBreaks, spaceWidth, aveCharWidth,
    0, 0,  -1, 0, &fonts, &offsets 
  };

  metrics->ResolveForwards(mDC, aString, aLength, do_BreakGetDimensions, &data);
  ...
  PRInt32 i = 1, breakIndex = 0;

  for (; i < count; ++i) {
    // Find the break index next to the last word that may use this font
    PRUnichar* offset = (PRUnichar*)offsets[i];
    while (aString + aBreaks[breakIndex] <= offset)
      ++breakIndex;
    ...
    // from a font different to that of the previous word. If 'x', 'y', 'z',
    ...
    // If we don't do this, a 'tall' trailing whitespace may cause the next line
    // to flicker when the window is slowly resized to fit that whitespace.
    if (aBreaks[breakIndex] < aLength && aString[aBreaks[breakIndex]] == ' ') {
      continue;
    }

    fontWin = (nsFontWin*)fonts.ElementAt(i);
    if (aBreaks[breakIndex] < aNumCharsFit) {
    ...
    else if (aBreaks[breakIndex] < aLength) {
      // The width of the last word is included in the final width,
      // but its ascent and descent are kept aside for the moment...
      // We do this because line-breaking may occur _before_ the last
      // word, and we don't want its ascent and descent to interfere.
      // We can later re-measure the last word and substract its width.
      // However, we need this special handling to recover the ascent
      // and descent at the break point. XXX What about BiDi?
  ...
  // This should be set as -1 to reply that we don't know the
  // width of the last word since we measure multiple words
  aLastWordDimensions.width = -1;
...
nsRenderingContextWin::GetDimensions(const char*   aString,
                                     ...
                                     nsDimensions& aDimensions)
{
  GetWidth(aString, aLength, aDimensions.width); // also set up the font
  aDimensions.ascent = mCurrFontWin->mMaxAscent;
  aDimensions.descent = mCurrFontWin->mMaxDescent;
  return NS_OK;
}

struct GetDimensionsData {
  HDC     mDC;      // IN
  HFONT   mFont;    // IN/OUT (running)
...
static PRBool PR_CALLBACK
do_GetDimensions(const nsFontSwitch* aFontSwitch,
                 const PRUnichar*    aSubstring,
                 PRUint32            aSubstringLength,
                 void*               aData)
{
  ...
  GetDimensionsData* data = (GetDimensionsData*)aData;
  if (data->mFont != fontWin->mFont) {
    data->mFont = fontWin->mFont;
    ::SelectObject(data->mDC, data->mFont);
  }
                                     ...
                                     PRInt32*         aFontID)
{
  ...
  GetDimensionsData data = {mDC, mCurrFont, 0, 0, 0};

  metrics->ResolveForwards(mDC, aString, aLength, do_GetDimensions, &data);
  ...
  if (mCurrFont != data.mFont) {
    // If the font was changed on th way, restore our font
    ::SelectObject(mDC, mCurrFont);
  }
