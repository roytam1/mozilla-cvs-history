...
void
nsFontMetricsWin::FillLogFont(LOGFONT* logFont, PRInt32 aWeight, PRBool aSizeOnly)
{
...
{
  // Get font metrics - XXX maybe, not all these should be cached
  float dev2app;
  ...
  if (0 < ::GetOutlineTextMetrics(aDC, sizeof(oMetrics), &oMetrics)) {
    // aFont->mXHeight = NSToCoordRound(oMetrics.otmsXHeight * dev2app);  XXX not really supported on windows
    aFont->mXHeight = NSToCoordRound((float)metrics.tmAscent * dev2app * 0.56f); // 50% of ascent, best guess for true type
    aFont->mSuperscriptOffset = NSToCoordRound(oMetrics.otmptSuperscriptOffset.y * dev2app);
    aFont->mSubscriptOffset = NSToCoordRound(oMetrics.otmptSubscriptOffset.y * dev2app);

    aFont->mStrikeoutSize = PR_MAX(onePixel, NSToCoordRound(oMetrics.otmsStrikeoutSize * dev2app));
    aFont->mStrikeoutOffset = NSToCoordRound(oMetrics.otmsStrikeoutPosition * dev2app);
    aFont->mUnderlineSize = PR_MAX(onePixel, NSToCoordRound(oMetrics.otmsUnderscoreSize * dev2app));
    aFont->mUnderlineOffset = NSToCoordRound(oMetrics.otmsUnderscorePosition * dev2app);
    ...
    if (GDI_ERROR != len && gm.gmptGlyphOrigin.y > 0) {
      aFont->mXHeight = NSToCoordRound(gm.gmptGlyphOrigin.y * dev2app);
    }
    ...
    aFont->mXHeight = NSToCoordRound((float)metrics.tmAscent * dev2app * 0.56f); // 56% of ascent, best guess for non-true type
    aFont->mSuperscriptOffset = aFont->mXHeight;     // XXX temporary code!
    aFont->mSubscriptOffset = aFont->mXHeight;     // XXX temporary code!

    aFont->mStrikeoutSize = onePixel; // XXX this is a guess
    aFont->mStrikeoutOffset = NSToCoordRound(aFont->mXHeight / 2.0f); // 50% of xHeight
    aFont->mUnderlineSize = onePixel; // XXX this is a guess
    aFont->mUnderlineOffset = -NSToCoordRound((float)metrics.tmDescent * dev2app * 0.30f); // 30% of descent
  }

  aFont->mLeading = NSToCoordRound(metrics.tmInternalLeading * dev2app);
  aFont->mEmHeight = NSToCoordRound((metrics.tmHeight - metrics.tmInternalLeading) *
                             dev2app);
  aFont->mEmAscent = NSToCoordRound((metrics.tmAscent - metrics.tmInternalLeading) *
                             dev2app);
  aFont->mEmDescent = NSToCoordRound(metrics.tmDescent * dev2app);
  aFont->mMaxHeight = NSToCoordRound(metrics.tmHeight * dev2app);
  aFont->mMaxAscent = NSToCoordRound(metrics.tmAscent * dev2app);
  aFont->mMaxDescent = NSToCoordRound(metrics.tmDescent * dev2app);
  aFont->mMaxAdvance = NSToCoordRound(metrics.tmMaxCharWidth * dev2app);
  aFont->mAveCharWidth = PR_MAX(1, NSToCoordRound(metrics.tmAveCharWidth * dev2app));

   // Cache the width of a single space.
  SIZE  size;
  ::GetTextExtentPoint32(aDC, " ", 1, &size);
  aFont->mSpaceWidth = NSToCoordRound(size.cx * dev2app);
}
...
nsFontMetricsWin::CreateFontHandle(HDC aDC, LOGFONT* aLogFont)
{
  ...
  if (hfont) {
    ...
    if (::GetTextFace(aDC, sizeof(name), name) &&
      ...
      // restore the unadjusted values before leaving
      mFont.size = baseSize;
...
#define MAKE_FONT_PREF_KEY(_pref, _s0, _s1, _s2, _s3, _s4) \
 _pref.Assign(_s0); \
 _pref.Append(_s1); \
 _pref.Append(_s2); \
 _pref.Append(_s3); \
...
nsFontMetricsWin::FindGenericFont(HDC aDC, PRUnichar aChar)
{
  ...
  // Get the fonts in the document's language group  
  // . font.current.[langGroup].[generic].name
  // . font.default.[langGroup].[generic].name

  if (mLangGroup) {
    nsAutoString tmp;
    mLangGroup->ToString(tmp);
    langGroup.Assign(NS_ConvertUCS2toUTF8(tmp));

    // font.current.[langGroup].[generic].name:
    // the current user's selected font, it gives the first preferred font
    MAKE_FONT_PREF_KEY(pref, "font.current.", langGroup, '.', generic, ".name");
    res = gPref->CopyUnicharPref(pref, getter_Copies(value));      
    if (NS_SUCCEEDED(res)) {
    ...
    MAKE_FONT_PREF_KEY(pref, "font.default.", langGroup, '.', generic, ".name");
    res = gPref->CopyUnicharPref(pref, getter_Copies(value));      
    if (NS_SUCCEEDED(res)) {
  ...
  // . font.default.[locale's langGroup].[generic].name

  if (gUsersLocale != mLangGroup) {
    nsAutoString tmp;
    gUsersLocale->ToString(tmp);
    langGroup.Assign(NS_ConvertUCS2toUTF8(tmp));
    MAKE_FONT_PREF_KEY(pref, "font.default.", langGroup, '.', generic, ".name");
    res = gPref->CopyUnicharPref(pref, getter_Copies(value));      
    if (NS_SUCCEEDED(res)) {
...
#ifdef DEBUG_rbs
  nsCAutoString family(NS_ConvertUCS2toUTF8(mFont.name));
  printf("No generic:%s mFont.name:%s\n", generic.get(), family.get());
#endif
...
FontEnumCallback(const nsString& aFamily, PRBool aGeneric, void *aData)
{
  ...
  if (aGeneric) {
    metrics->mFonts.StringAt(metrics->mFonts.Count()-1, metrics->mGeneric);
...
nsFontMetricsWin::RealizeFont()
{
  ...
  // set a fallback generic font name if the font-family list didn't have one
  if (mGeneric.IsEmpty() && mLangGroup) {
    nsAutoString tmp;
    nsCAutoString langGroup;
    mLangGroup->ToString(tmp);
    langGroup.Assign(NS_ConvertUCS2toUTF8(tmp));

    pref.Assign("font.current.");
    pref.Append(langGroup);
    pref.Append(".generic.name");
    rv = gPref->CopyUnicharPref(pref, getter_Copies(value));
    if (NS_SUCCEEDED(rv)) {
  ...
  if (mLangGroup.get() == gUserDefined) {
    ...
    // font.current.[USER_DEFINED].[generic].name
    pref.Assign("font.current.x-user-def.");
  ...
  // -------------------------------------------------------------------
  // The computed CSS metrics is always unadjusted....
  // We need to retain the unadjusted metrics.

  if (mFont.sizeAdjust <= 0) {
    // When no adjustment happens, we get the metrics directly
    mLeading = font->mLeading;
    mEmHeight = font->mEmHeight;
    mEmAscent = font->mEmAscent;
    mEmDescent = font->mEmDescent;
    mMaxHeight = font->mMaxHeight;
    mMaxAscent = font->mMaxAscent;
    mMaxDescent = font->mMaxDescent;
    mMaxAdvance = font->mMaxAdvance;       
    mAveCharWidth = font->mAveCharWidth;
    mXHeight = font->mXHeight;
    mSuperscriptOffset = font->mSuperscriptOffset;
    mSubscriptOffset = font->mSubscriptOffset;
    mStrikeoutSize = font->mStrikeoutSize;
    mStrikeoutOffset = font->mStrikeoutOffset;
    mUnderlineSize = font->mUnderlineSize;
    mUnderlineOffset = font->mUnderlineOffset;
    mSpaceWidth = font->mSpaceWidth;
  ...
  // Recover the unadjusted font
  PRInt32 dummy = 0;
  LOGFONT logFont;
  ::GetObject(font->mFont, sizeof(LOGFONT), &logFont);
  FillLogFont(&logFont, dummy, PR_TRUE);

  HFONT hfont = ::CreateFontIndirect(&logFont);
  if (hfont) {
    HFONT oldFont = (HFONT)::SelectObject(dc, (HGDIOBJ)hfont);
    char name[sizeof(logFont.lfFaceName)];
    if (::GetTextFace(dc, sizeof(name), name) &&
        !strcmpi(name, logFont.lfFaceName)) {
      // Now get the unadjusted font metrics
      float dev2app;
      mDeviceContext->GetDevUnitsToAppUnits(dev2app);
      nscoord onePixel = NSToCoordRound(1 * dev2app);
      OUTLINETEXTMETRIC oMetrics;
