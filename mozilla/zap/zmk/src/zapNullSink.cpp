...
NS_IMPL_THREADSAFE_ADDREF(zapNullSink)
NS_IMPL_THREADSAFE_RELEASE(zapNullSink)

NS_INTERFACE_MAP_BEGIN(zapNullSink)
...
NS_IMETHODIMP
zapNullSink::InsertedIntoContainer(zapIMediaNodeContainer *container,
...
NS_IMETHODIMP
zapNullSink::RemovedFromContainer(zapIMediaNodeContainer *container)
{
  return NS_OK;
}
...
NS_IMETHODIMP
zapNullSink::GetSource(nsIPropertyBag2 *source_pars, zapIMediaSource **_retval)
{
  NS_ERROR("sink-only node");
  return NS_ERROR_FAILURE;  
}

/* zapIMediaSink getSink (in nsIPropertyBag2 sink_pars); */
NS_IMETHODIMP
zapNullSink::GetSink(nsIPropertyBag2 *sink_pars, zapIMediaSink **_retval)
{
  if (mInput) {
...
NS_IMETHODIMP
zapNullSink::ConnectSource(zapIMediaSource *source)
{
...
NS_IMETHODIMP
zapNullSink::DisconnectSource(zapIMediaSource *source)
{
  mInput = nsnull;
  return NS_OK;
}
...
NS_IMETHODIMP
zapNullSink::ConsumeFrame(zapIMediaFrame * frame)
{
