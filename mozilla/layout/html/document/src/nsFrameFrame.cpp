                                        ...
                                        PRInt32 aHint)
{
  ...
  if (nsHTMLAtoms::noresize == aAttribute) {
    ...
    //    nsCOMPtr<nsIFrameElement> frameElement(do_QueryInterface(mContent));
    //    if (!frameElement)
    //      return NS_OK;
    ...
    //    frameElement->GetDocShell(getter_AddRefs(docShell));
...
nsHTMLFrameInnerFrame::~nsHTMLFrameInnerFrame()
{
  ...
  if (xulElt && docShell) {
    // We might be a XUL browser and may need to store the current URL
    // in our box object.
    ...
    if (boxObject) {
      ...
      if (browser) {
        nsCOMPtr<nsIWebNavigation> webShell(do_QueryInterface(docShell));
        nsCOMPtr<nsISHistory> hist;

        webShell->GetSessionHistory(getter_AddRefs(hist));

        if (hist)
          boxObject->SetPropertyAsSupports(NS_LITERAL_STRING("history").get(),
                                           hist);
        nsCOMPtr<nsISupports> supports;
        boxObject->GetPropertyAsSupports(NS_LITERAL_STRING("listenerkungfu").get(),
                                         getter_AddRefs(supports));
        if (supports)
          boxObject->SetPropertyAsSupports(NS_LITERAL_STRING("listener").get(),
                                           supports);
      }
    }
  }
  ...
  if (docShell) {
    ...
    if (doc) {
      doc->SetSubDocumentFor(mContent, nsnull);
    }
...
nsHTMLFrameInnerFrame::ShowDocShell(nsIPresContext* aPresContext)
{
  ...
  /*
  nsCOMPtr<nsIDocument> doc;
  parentPresShell->GetDocument(getter_AddRefs(doc));

  parentDoc->SetSubDocumentFor(content, doc);
  */
  ...
  if (scrollableContainer) {
    scrollableContainer->
      SetDefaultScrollbarPreferences(nsIScrollable::ScrollOrientation_Y,
                                     GetScrolling(content, mode));

    scrollableContainer->
      SetDefaultScrollbarPreferences(nsIScrollable::ScrollOrientation_X,
                                     GetScrolling(content, mode));
  }
  ...
  // base_win->SetParentWidget(nsnull) should be done in this file...
  ...
  nsCOMPtr<nsIContentViewer> content_viewer;
  docShell->GetContentViewer(getter_AddRefs(content_viewer));
  NS_ENSURE_TRUE(content_viewer, NS_ERROR_UNEXPECTED);

  content_viewer->SetWidget(widget);
