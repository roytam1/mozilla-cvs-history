...
#ifdef USE_IMG2
NS_IMPL_ISUPPORTS2(nsImgListener, imgIDecoderObserver, gfxIImageContainerObserver)
...
NS_IMETHODIMP nsImgListener::OnStartFrame(imgIRequest *request, nsISupports *cx, gfxIImageFrame *frame)
{
  nsCOMPtr<nsIPresContext> pc(do_QueryInterface(cx));
  return mFrame->OnStartFrame(request, pc, frame);
}

NS_IMETHODIMP nsImgListener::OnDataAvailable(imgIRequest *request, nsISupports *cx, gfxIImageFrame *frame, const nsRect * rect)
{
  nsCOMPtr<nsIPresContext> pc(do_QueryInterface(cx));
  return mFrame->OnDataAvailable(request, pc, frame, rect);
}

NS_IMETHODIMP nsImgListener::OnStopFrame(imgIRequest *request, nsISupports *cx, gfxIImageFrame *frame)
{
  nsCOMPtr<nsIPresContext> pc(do_QueryInterface(cx));
...
NS_IMETHODIMP nsImgListener::FrameChanged(gfxIImageContainer *container, nsISupports *cx, gfxIImageFrame *newframe, nsRect * dirtyRect)
{
  nsCOMPtr<nsIPresContext> pc(do_QueryInterface(cx));
