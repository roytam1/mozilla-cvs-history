...
{
  // Create a CSS loader that we'll use for loading all of the sheets.
  mCSSLoader = do_CreateInstance(kCSSLoaderCID);
...
nsStyleSheetService::LoadAndRegisterSheet(nsIURI *aSheetURI,
                                          PRUint32 aSheetType)
{
  NS_ASSERTION(mCSSLoader, "CSS loader must exist");

  nsCOMPtr<nsICSSStyleSheet> sheet;
  nsresult rv = mCSSLoader->LoadAgentSheet(aSheetURI, getter_AddRefs(sheet));
