...
function onfocus(e)
{
    function debounce()
    {
        ...
        delete client.focusDebounceHandle;
    };
    
    var elem = document.commandDispatcher.focusedElement;
    var singleInput = document.getElementById("input");
    if (elem && document.getBindingParent(elem) == singleInput)
        return;

    dd(e.originalTarget);
    
    if ("focusDebounceHandle" in client)
        clearTimeout(client.focusDebounceHandle);
    
    client.focusDebounceHandle = setTimeout(debounce, 250);
}
...
function onMessageViewClick(e)
{
    if (!document.commandDispatcher.focusedElement &&
        !client.prefs["allowMsgViewFocus"])
    {
        dispatch("focus-input");
    }
