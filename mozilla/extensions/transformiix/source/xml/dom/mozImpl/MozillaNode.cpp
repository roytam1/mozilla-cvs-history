...
Node* Node::getParentNode()
{
  ...
  if (nsNode->GetParentNode(&tmpParent) == NS_OK)
    return ownerDocument->createNode(tmpParent);
  else
    return NULL;
}
...
Node* Node::getFirstChild()
{
  ...
  if (nsNode->GetFirstChild(&tmpFirstChild) == NS_OK)
    return ownerDocument->createNode(tmpFirstChild);
  else
    return NULL;
}
...
Node* Node::getLastChild() 
{
  ...
  if (nsNode->GetLastChild(&tmpLastChild) == NS_OK)
    return ownerDocument->createNode(tmpLastChild);
  else
    return NULL;
}
...
Node* Node::getPreviousSibling()
{
  ...
  if (nsNode->GetPreviousSibling(&tmpPrevSib) == NS_OK)
    return ownerDocument->createNode(tmpPrevSib);
  else
    return NULL;
}
...
Node* Node::getNextSibling()
{
  ...
  if (nsNode->GetNextSibling(&tmpNextSib) == NS_OK)
    return ownerDocument->createNode(tmpNextSib);
  else
    return NULL;
}
                         ...
                         Node* refChild)
{
  ...
  if (nsNode->InsertBefore(newChild->getNSObj(), refChild->getNSObj(),
			   &returnValue) == NS_OK)
    return ownerDocument->createNode(returnValue);
  else
    return NULL;
}
