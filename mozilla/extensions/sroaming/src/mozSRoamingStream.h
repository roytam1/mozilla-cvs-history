...
/* This implementation uses Necko to transfer files to/from the server.
   ...
   implementation in Necko. Currently, that includes HTTP and FTP.
*/

#ifndef _MOZSROAMINGSTREAM_H_
#define _MOZSROAMINGSTREAM_H_

#include "mozSRoamingProtocol.h"

class mozSRoamingStream: public mozSRoamingProtocol
{
public:
    mozSRoamingStream();
    virtual ~mozSRoamingStream();
...
protected:
    // Data
    nsString mRemoteBaseUrl;  /* Directory on server, where profile files are.
                                 This is already with the concrete username. */
    nsString mPassword;
    PRBool mSavePassword;     /* If we have to ask for a password, this is
