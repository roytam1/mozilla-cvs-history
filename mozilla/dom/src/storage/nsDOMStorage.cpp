...
// XXXjst: This should return an already_AddRefed()!
                            ...
                            nsIDOMStorage** aStorage)
{
  ...
  if (ssm) {
    ...
    if (subjectPrincipal == systemPrincipal || !currentDomain.IsEmpty()) {
      rv = GetStorageForDomain(aDomain, NS_ConvertUTF8toUTF16(currentDomain),
...
nsDOMStorageItem::GetValue(nsAString& aValue)
{
  if (mStorage) {
    ...
    PRBool secure;
    return mStorage->GetDBValue(mKeyOrValue, aValue, &secure);
  }
...
{
  return InitEvent(NS_LITERAL_STRING("storage"), PR_TRUE, PR_FALSE);
}
