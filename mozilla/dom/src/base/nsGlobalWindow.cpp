...
nsGlobalWindow::GetSessionStorage(nsIDOMStorage ** aSessionStorage)
{
  *aSessionStorage = nsnull;
  ...
  nsCOMPtr<nsIURI> uri = curDoc->GetDocumentURI();
                        ...
                        const PRUnichar *someData)
{
  if (IsInnerWindow() && !nsCRT::strcmp(aTopic, "dom-storage-changed")) {
    ...
    if (!someData) {
      ...
      nsCAutoString currentDomain;
      rv = curDoc->GetDocumentURI()->GetAsciiHost(currentDomain);
      if (NS_FAILED(rv)) {
        return NS_OK;
      }
                             ...
                             nsIDOMWindow **aReturn)
{
  ...
  *aReturn = nsnull;
  ...
  // copy the session storage data over to the new window if necessary.
  ...
  if (newDocShell && thisWebNav) {
    nsCOMPtr<nsIURI> thisURI, newURI;
    thisWebNav->GetCurrentURI(getter_AddRefs(thisURI));
    ...
    if (thisURI && newURI) {
      ...
      thisURI->GetAsciiHost(thisDomain);
