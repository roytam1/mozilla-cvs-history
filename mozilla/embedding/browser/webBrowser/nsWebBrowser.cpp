...
nsWebBrowser::nsWebBrowser() : mInitInfo(nsnull), mParentNativeWindow(nsnull),
   mTopLevelWindow(nsnull), mParentWidget(nsnull), mParent(nsnull)
{
	NS_INIT_REFCNT();
...
/*   if(aIID.Equals(NS_GET_IID(nsIURIContentListener)) &&
      ...
      *aSink = mContentListener;
      ...
      {
      nsCOMPtr<nsIInterfaceRequestor> requestor(do_QueryInterface(mDocShell));
...
{
   mTopLevelWindow = aTopWindow;  //Weak Reference
   return NS_OK;
}
...
NS_IMETHODIMP nsWebBrowser::SetVisibility(PRBool visibility)
{
   if(!mDocShell)
      ...
      NS_ENSURE_SUCCESS(mDocShellAsWin->SetVisibility(visibility), NS_ERROR_FAILURE);
...
NS_IMETHODIMP nsWebBrowser::SetDocShell(nsIDocShell* aDocShell)
{
   if(aDocShell)
      ...
      mDocShell = aDocShell;
      mDocShellAsWin = do_QueryInterface(aDocShell);
      mDocShellAsItem = do_QueryInterface(aDocShell);
      mDocShellAsNav = do_QueryInterface(aDocShell);
      mDocShellAsScrollable = do_QueryInterface(aDocShell);
      mDocShellAsTextScroll = do_QueryInterface(aDocShell);
      NS_ENSURE_TRUE(mDocShell && mDocShellAsWin && mDocShellAsItem &&
         mDocShellAsNav && mDocShellAsScrollable && mDocShellAsTextScroll,
         NS_ERROR_FAILURE);
      }
   else
      {
