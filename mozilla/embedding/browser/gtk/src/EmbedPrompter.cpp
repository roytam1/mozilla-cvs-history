...
EmbedPrompter::Create(PromptType aType, GtkWindow* aParentWindow)
{
    ...
    gtk_box_set_spacing(GTK_BOX(GTK_DIALOG(mWindow)->vbox), 5);

    // create a hbox for the icon and the message text
    GtkWidget* labelBox = gtk_hbox_new(FALSE, 5);
    ...
    // put the label in the box, and pack the box into the larger box
    GtkWidget* label = gtk_label_new(mMessageText.get());
    gtk_label_set_line_wrap(GTK_LABEL(label), TRUE);
    gtk_box_pack_start(GTK_BOX(labelBox), label, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(mWindow)->vbox), labelBox,
                       FALSE, FALSE, 0);
    ...
    if (widgetFlags & INCLUDE_USERNAME) {
        ...
        gtk_box_pack_start(GTK_BOX(GTK_DIALOG(mWindow)->vbox), userLabel,
                           FALSE, FALSE, 0);
        ...
        gtk_box_pack_start(GTK_BOX(GTK_DIALOG(mWindow)->vbox), mUserField,
                           FALSE, FALSE, 0);
    }
    if (widgetFlags & INCLUDE_PASSWORD) {
        GtkWidget* passLabel = gtk_label_new("Password");
        gtk_box_pack_start(GTK_BOX(GTK_DIALOG(mWindow)->vbox), passLabel,
                           FALSE, FALSE, 0);
        ...
        gtk_box_pack_start(GTK_BOX(GTK_DIALOG(mWindow)->vbox), mPassField,
                           FALSE, FALSE, 0);
    }
    if (aType == TYPE_PROMPT) {
        ...
        gtk_box_pack_start(GTK_BOX(GTK_DIALOG(mWindow)->vbox), mTextField,
                           FALSE, FALSE, 0);
    }
    ...
    if ((widgetFlags & INCLUDE_CHECKBOX) && !mCheckMessage.IsEmpty()) {
        ...
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(mCheckBox),
                                     mCheckValue);
        gtk_box_pack_start(GTK_BOX(GTK_DIALOG(mWindow)->vbox), mCheckBox,
                           FALSE, FALSE, 0);
    }
    ...
    if (aType == TYPE_SELECT) {
        ...
        gtk_box_pack_start(GTK_BOX(GTK_DIALOG(mWindow)->vbox), mOptionMenu,
                           FALSE, FALSE, 0);
    }
