...
NS_IMETHODIMP
nsCommandManager::GetCommandState(const nsAString & aCommandName, /* in out */ nsICommandParams **aCommandParams)
{
  nsCOMPtr<nsIController> controller;
  nsresult rv = GetControllerForCommand(aCommandName, getter_AddRefs(controller)); 
  if (!controller)
...
NS_IMETHODIMP
nsCommandManager::DoCommand(const nsAString & aCommandName, /* const */ nsICommandParams *aCommandParams)
{
  nsCOMPtr<nsIController> controller;
  nsresult rv = GetControllerForCommand(aCommandName, getter_AddRefs(controller)); 
  if (!controller)
