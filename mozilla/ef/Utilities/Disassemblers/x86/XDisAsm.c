...
#define db_printsym(val, flags) disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "0x%08x", val)


static char disasm_buff[128];
static char *disasm_next;
static char *disasm_end = disasm_buff + sizeof(disasm_buff);
...
{
	if (addrp->is_reg) {
	    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s", db_reg[size][addrp->disp]);
	    return;
	}

	if (seg) {
	    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s:", seg);
	}

	db_printsym((db_addr_t)addrp->disp, DB_STGY_ANY);
	if (addrp->base != 0 || addrp->index != 0) {
	    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "(");
	    if (addrp->base)
		disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s", addrp->base);
	    if (addrp->index)
		disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, ",%s,%d", addrp->index, 1<<addrp->ss);
	    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, ")");
	}
}
	...
	const char *	seg;
{
	...
	if (mod != 3) {
	    if (*fp->f_name == '\0') {
		disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "<bad instruction>");
		return (loc);
	    }
	    /*
	    ...
	    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, fp->f_name);
	    switch(fp->f_size) {
		case SNGL:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "s");
		    break;
		case DBLR:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "l");
		    break;
		case EXTR:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "t");
		    break;
		case WORD:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "s");
		    break;
		case LONG:
		    ...
		    break;
		case QUAD:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "q");
		    break;
		default:
		    break;
	    }
		...
		case op2(ST,STI):
		    ...
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s\t%%st,%%st(%d)",name,f_rm(regmodrm));
		    break;
		case op2(STI,ST):
		    name = (fp->f_rrname) ? fp->f_rrname : fp->f_name;
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s\t%%st(%d),%%st",name, f_rm(regmodrm));
		    break;
		case op1(STI):
		    name = (fp->f_rrname) ? fp->f_rrname : fp->f_name;
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s\t%%st(%d)",name, f_rm(regmodrm));
		    break;
		case op1(X):
		    name = ((const char * const *)fp->f_rrname)[f_rm(regmodrm)];
		    if (*name == '\0')
			goto bad;
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s", name);
		    break;
		case op1(XA):
		    name = ((const char * const *)fp->f_rrname)[f_rm(regmodrm)];
		    if (*name == '\0')
			goto bad;
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s\t%%ax", name);
		    break;
		default:
		bad:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "<bad instruction>");
...
db_disasm(db_addr_t loc)
{
	...
	do {
		...
		case 0x65:
		    ...
		    break;
		case 0xf0:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "lock ");
		    break;
		case 0xf2:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "repne ");
		    break;
		case 0xf3:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "repe ");	/* XXX repe VS rep */
		    break;
		default:
		    prefix = FALSE;
		    break;
	...
	if (i_size == SDEP) {
	    if (size == WORD)
		disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, i_name);
	    else
		disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, (const char *)ip->i_extra);
	}
	else {
	    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, i_name);
	    if (i_size != NONE) {
		if (i_size == BYTE) {
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "b");
		    size = BYTE;
		}
		else if (i_size == WORD) {
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "w");
		    size = WORD;
		}
		else if (size == WORD)
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "w");
	     ...
	     i_mode >>= 8, first = FALSE)
	{
		...
		case R:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s", db_reg[size][f_reg(regmodrm)]);
		    break;

		case Rw:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s", db_reg[WORD][f_reg(regmodrm)]);
		    break;

		case Ri:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s", db_reg[size][f_rm(inst)]);
		    break;

		case Ril:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s", db_reg[LONG][f_rm(inst)]);
		    break;

		case S:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s", db_seg_reg[f_reg(regmodrm)]);
		    break;

		case Si:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s", db_seg_reg[f_reg(inst)]);
		    break;

		case A:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s", db_reg[size][0]);	/* acc */
		    break;

		case BX:
		    if (seg)
			disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s:", seg);
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "(%s)", short_addr ? "%bx" : "%ebx");
		    break;

		case CL:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%%cl");
		    break;

		case DX:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%%dx");
		    break;

		case SI:
		    if (seg)
			disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%s:", seg);
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "(%s)", short_addr ? "%si" : "%esi");
		    break;

		case DI:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%%es:(%s)", short_addr ? "%di" : "%edi");
		    break;

		case CR:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%%cr%d", f_reg(regmodrm));
		    break;

		case DR:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%%dr%d", f_reg(regmodrm));
		    break;

		case TR:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "%%tr%d", f_reg(regmodrm));
		    break;
		...
		case o1:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "$1");
		    break;

		case o3:
		    disasm_next += PR_snprintf(disasm_next, disasm_end - disasm_next, "$3");
		    break;
...
{
    *startByte = (char*)db_disasm((db_addr_t)*startByte);
    return &disasm_buff[0];
}
