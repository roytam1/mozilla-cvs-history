...
nsSecureBrowserUIImpl::Init(nsIDOMWindow *window, nsIDOMElement *button)
{
	...
	if (sgo)
	{
      nsCOMPtr<nsIDocShell> docShell;
		...
		nsCOMPtr<nsIWebShell> ws(do_QueryInterface(docShell));
		if (ws)
		{
			...
			{
				NS_ADDREF(sbo);
				return sbo->Init(button, ws); // does the window delete us when it close?
			}
		}
	}
	return NS_OK;
...
nsresult
nsSecureBrowserObserver::Init(nsIDOMElement *button, nsIWebShell* content)
{
	if (!button || !content)
...
NS_IMETHODIMP
nsSecureBrowserObserver::HandleUnknownContentType( nsIDocumentLoader* loader, 
                                             nsIChannel* channel, 
                                             ...
                                             const char *aCommand )
{
    if (mOldWebShellObserver)
	{
		mOldWebShellObserver->HandleUnknownContentType(loader, channel, aContentType, aCommand);
	}
