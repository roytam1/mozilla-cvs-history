...
#define CSS_PROP_SVGRESET(name_, id_, method_, hint_, datastruct_, member_, type_, iscoord_) \
  { offsetof(nsRuleData##datastruct_, member_), type_, iscoord_ },
#include "nsCSSPropList.h"
...
const nsStyleStruct*
nsRuleNode::GetSVGResetData(nsStyleContext* aContext, PRBool aComputeData)
{
  nsRuleDataSVG svgData; // Declare a struct with null CSS values.
  nsRuleData ruleData(eStyleStruct_SVGReset, mPresContext, aContext);
  ruleData.mSVGData = &svgData;

  return WalkRuleTree(eStyleStruct_SVGReset, aContext, &ruleData, &svgData, aComputeData);
}
#endif
