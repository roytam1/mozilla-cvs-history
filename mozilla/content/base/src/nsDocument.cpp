...
//*****************************************************************************
// nsDocument::SetDocumentBidi
//*****************************************************************************   

NS_IMETHODIMP
nsDocument::SetDocumentBidi(const PRUint8 member, const PRUint8 value)
{
	// -------------------------------------
	// IBM Bidi prefs
	// -------------------------------------
  ...
  PRInt32 i;
  PRInt32 ns = this->GetNumberOfShells();
  for (i = 0; i < ns; i++) 
  {
    nsCOMPtr<nsIPresShell> shell(dont_AddRef(this->GetShellAt(i)));
    if (shell) 
    {
			//nsCOMPtr<nsIContentViewer> documentViewer;
      // Resize-reflow this time
      //nsCOMPtr<nsIDocumentViewer> docViewer = do_QueryInterface(mRootContent);//documentViewer
      //if (!docViewer) { return NS_ERROR_OUT_OF_MEMORY; }
      nsCOMPtr<nsIPresContext> presContext = nsnull;
      //NS_ENSURE_SUCCESS(docViewer->GetPresContext(*(getter_AddRefs(presContext))), NS_ERROR_FAILURE);
			NS_ENSURE_SUCCESS(shell->GetPresContext(getter_AddRefs(presContext)), NS_ERROR_FAILURE);
      if (!presContext) { return NS_ERROR_OUT_OF_MEMORY; }

			presContext->SetDocumentBidi(member, value);
    }
  }
...
NS_IMETHODIMP
nsDocument::GetDocumentBidi(const PRUint8 member, PRUint8 * value)
{
	// -------------------------------------
	// IBM Bidi prefs
	// -------------------------------------
