...
nsHTMLContentSerializer::nsHTMLContentSerializer()
{
  ...
  mIndent = 0;
  mAddSpace = PR_FALSE;
  mMayEatLineBreakSequence = PR_FALSE;
  mInBody = PR_FALSE;
  mInCDATA = PR_FALSE;
}
                                               ...
                                               PRBool aTranslateEntities)
{
  ...
  PRBool mayIgnoreInitialWhitespaceSequence = !mColPos;
  ...
  while (pos < end) {
    ...
    if (' ' == c || '\n' == c || '\t' == c) {
      ...
      // handle the complete sequence of whitespace in this block
      ...
      else if (!sawBlankOrTab && mMayEatLineBreakSequence) {
        // nothing to do
        mMayEatLineBreakSequence = PR_FALSE;
      }
      else if (mayIgnoreInitialWhitespaceSequence) { 
        // nothing to do
        mayIgnoreInitialWhitespaceSequence = PR_FALSE;
      }
      else {
        if (sawBlankOrTab) {
          ...
          aOutputStr.Append(mLineBreak);
          mMayEatLineBreakSequence = PR_TRUE;
          mColPos = 0;
        }
      }
    }
      ...
      do {
        ...
        // iterate until we find the first whitespace char or an end condition

        PRBool foundWhitespaceInLoop = PR_FALSE;
          ...
          else 
          {
            ...
            if (mLineBreaker) { // we have a line breaker helper object
              ...
              if (foundWrapPosition) {
                ...
                aOutputStr.Append(sequenceStart, wrapPosition);
                aOutputStr.Append(mLineBreak);
                pos = sequenceStart + wrapPosition;
                mColPos = 0;
                mayIgnoreInitialWhitespaceSequence = PR_TRUE;
                mMayEatLineBreakSequence = PR_TRUE;
              }
            }
