...
nsXTFXMLVisualWrapper::QueryInterface(REFNSIID aIID, void** aInstancePtr)
{
  ...
  else if (NS_SUCCEEDED(rv = nsXTFXMLVisualWrapperBase::QueryInterface(aIID, aInstancePtr))) {
    return rv;
  }
...
nsresult
nsXTFXMLVisualWrapper::GetAttr(PRInt32 aNameSpaceID, nsIAtom* aName, 
                               nsAString& aResult)const
{
  nsAutoString name;
  aName->ToString(name);
...
PRBool
nsXTFXMLVisualWrapper::HasAttr(PRInt32 aNameSpaceID, nsIAtom* aName) const
{
  nsAutoString name;
  aName->ToString(name);
...
nsresult
nsXTFXMLVisualWrapper::GetAttrNameAt(PRUint32 aIndex, PRInt32* aNameSpaceID,
                                     nsIAtom** aName, nsIAtom** aPrefix) const
{
...
PRUint32
nsXTFXMLVisualWrapper::GetAttrCount() const
{
  PRUint32 rval = 0;
