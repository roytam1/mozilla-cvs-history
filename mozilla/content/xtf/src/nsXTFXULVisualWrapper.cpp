...
void
nsXTFXULVisualWrapper::SetDocument(nsIDocument* aDocument, PRBool aDeep,
                                   PRBool aCompileEventHandlers)
{
  mXTFElement->WillChangeDocument(aDocument);
  nsXTFXULVisualWrapperBase::SetDocument(aDocument, aDeep, aCompileEventHandlers);
  mXTFElement->DocumentChanged(aDocument);
}

void
nsXTFXULVisualWrapper::SetParent(nsIContent* aParent)
{
  mXTFElement->WillChangeParent(aParent);
  nsXTFXULVisualWrapperBase::SetParent(aParent);
  mXTFElement->ParentChanged(aParent);
}

nsresult
nsXTFXULVisualWrapper::InsertChildAt(nsIContent* aKid, PRUint32 aIndex,
                                     PRBool aNotify, PRBool aDeepSetDocument)
{
  nsresult rv;
  mXTFElement->WillInsertChild(aKid, aIndex);
  rv = nsXTFXULVisualWrapperBase::InsertChildAt(aKid, aIndex, aNotify, aDeepSetDocument);
  mXTFElement->ChildInserted(aKid, aIndex);
  return rv;
}

nsresult
nsXTFXULVisualWrapper::ReplaceChildAt(nsIContent* aKid, PRUint32 aIndex,
                                      PRBool aNotify, PRBool aDeepSetDocument)
{
  nsresult rv;
  mXTFElement->WillReplaceChild(aKid, aIndex);
  rv = nsXTFXULVisualWrapperBase::ReplaceChildAt(aKid, aIndex, aNotify, aDeepSetDocument);
  mXTFElement->ChildReplaced(aKid, aIndex);
  return rv;
}

nsresult
nsXTFXULVisualWrapper::AppendChildTo(nsIContent* aKid, PRBool aNotify,
                                     PRBool aDeepSetDocument)
{
  nsresult rv;
  mXTFElement->WillAppendChild(aKid);
  rv = nsXTFXULVisualWrapperBase::AppendChildTo(aKid, aNotify, aDeepSetDocument);
  mXTFElement->ChildAppended(aKid);
  return rv;
}

nsresult
nsXTFXULVisualWrapper::RemoveChildAt(PRUint32 aIndex, PRBool aNotify)
{
  nsresult rv;
  mXTFElement->WillRemoveChild(aIndex);
  rv = nsXTFXULVisualWrapperBase::RemoveChildAt(aIndex, aNotify);
  mXTFElement->ChildRemoved(aIndex);
  return rv;
}

nsIAtom *
nsXTFXULVisualWrapper::GetIDAttributeName() const
{
  // XXX:
...
nsresult
nsXTFXULVisualWrapper::SetAttr(PRInt32 aNameSpaceID, nsIAtom* aName,
                               nsIAtom* aPrefix, const nsAString& aValue,
                               PRBool aNotify)
{
  nsAutoString name;
...
NS_IMETHODIMP 
nsXTFXULVisualWrapper::GetInterfaces(PRUint32 *count, nsIID * **array)
{
...
NS_IMETHODIMP 
nsXTFXULVisualWrapper::GetHelperForLanguage(PRUint32 language, nsISupports **_retval)
{
  *_retval = nsnull;
  return NS_OK;
}
...
NS_IMETHODIMP 
nsXTFXULVisualWrapper::GetContractID(char * *aContractID)
{
  *aContractID = nsnull;
  return NS_OK;
}
...
NS_IMETHODIMP 
nsXTFXULVisualWrapper::GetClassDescription(char * *aClassDescription)
{
...
NS_IMETHODIMP 
nsXTFXULVisualWrapper::GetClassID(nsCID * *aClassID)
{
  *aClassID = nsnull;
  return NS_OK;
}
...
NS_IMETHODIMP 
nsXTFXULVisualWrapper::GetImplementationLanguage(PRUint32 *aImplementationLanguage)
{
...
NS_IMETHODIMP 
nsXTFXULVisualWrapper::GetFlags(PRUint32 *aFlags)
{
...
NS_IMETHODIMP 
nsXTFXULVisualWrapper::GetClassIDNoAlloc(nsCID *aClassIDNoAlloc)
{
...
NS_IMETHODIMP
nsXTFXULVisualWrapper::GetElementNode(nsIDOMElement * *aElementNode)
{
...
// 		NS_ASSERTION(contentDescriptorCount==1, "nsXTFXULVisualWrapper::CreateAnonymousContent: \
...
// void nsXTFXULVisualWrapper::CreateNode(nsIContent* parent, XTLContentDescriptor* descriptor,
...
// void nsXTFXULVisualWrapper::CreateContentElement(PRInt32 ns, nsIAtom* tagname, XTLAttributeDescriptor* attribs,
...
//   if (!elementFactory) {
//     NS_ERROR("no element factory in nsXTFXULVisualWrapper");
//     return;
//   }
...
// implementation helpers:
PRBool
nsXTFXULVisualWrapper::AggregatesInterface(REFNSIID aIID)
{
...
// #ifdef DEBUG
//       printf("nsXTFXULVisualWrapper::AggregatesInterface(): found!\n");
// #endif
//       break;
//     }
//   }
