...
nsDragHelperService::Leave ( DragReference inDragRef, nsIEventSink *inSink )
{
  ...
  if (currentDragSession) {
    ...
    if (!sourceNode) {
      // We're leaving a window while doing a drag that was initiated
      // in a different app. End the drag session, since we're done
      // with it for now (until the user drags back into mozilla).
