...
NET_AddExternalURLType(char * type)
{
	...
	PL_ListEnumReset(ExternalURLTypeList);
	while((reg_type = (char *)PL_ListEnumNext(ExternalURLTypeList)) != NULL)
	  {
	...
	if(!ExternalURLTypeList)
		ExternalURLTypeList = PL_ListNew();

	PL_ListAdd(ExternalURLTypeList, new_type);
    ...
    while((reg_type = (char *)PL_ListEnumNext(ExternalURLTypeList)) != NULL)
      {
        if(!PL_strcasecmp(reg_type, type))
		  {
			/* found it */
			PL_ListRemove(ExternalURLTypeList, reg_type);
            return; /* done */ 
		  }
      }
}
		...
		if(colon)
		  {
             ...
             */
            if(PL_IS_ALPHA(*cat_point) && *(cat_point+1) == ':')
                cat_point += 2;
#endif /* XP_WIN */
                         ...
                         char **relative_url )
{
    ...
    */
...
#else
        PL_TO_LOWER(*(url_ptr-1)) != PL_TO_LOWER(*(base_ptr-1)) )
#endif
    {
        Result = NET_URL_NOT_ON_SAME_DEVICE;
        goto CLEAN_UP;
    ...
    while ( 
            ...
            *base_ptr == *url_ptr && 
#else
            PL_TO_LOWER(*base_ptr) == PL_TO_LOWER(*url_ptr) &&
#endif
...
NET_HTTPIndexParserInit()
{
	...
	PL_memset(rv, 0, sizeof(HTTPIndexParserData));
