...
js_ValueToNumber(JSContext *cx, jsval v, jsdouble *dp)
{
    ...
    *dp = *JSVAL_TO_DOUBLE(v);
    ...
    errno = 0;
    if (!js_strtod(str->chars, &ep, &d) || *ep != 0)            /* decimal check */
        if (!js_strtol(str->chars, &ep, 16, &d) || *ep != 0) {  /* hexa check */
            sign=0;                                             /* Infinity check */
            s = str->chars;
            if (str->length==9) {
                switch (*s++) {
                case '-' : sign=1;
            ...
            if (str->length!=8)
                goto badstr;
            if (s[0]!='I'||         /* hard-coded, but unlikely to change */
                s[1]!='n'||
                s[2]!='f'||
                s[3]!='i'||
                s[4]!='n'||
                ...
                s[7]!='y')
                goto badstr;
            d=(sign)?*cx->runtime->jsNegativeInfinity:
