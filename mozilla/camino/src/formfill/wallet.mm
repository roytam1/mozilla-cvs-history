...
//
// This basically works by using all the same tables/schemas that the old wallet
// code used for form-fill, but the |wallet_SchemaToValue_list| is initialized from the
// "me" card of the address book. We shim into the init code to do this rather than
// reading from the data file. Also, since we're not saving any user data, the encryption
// routines have been paired back to just doing a strdup.
//
...
static void wallet_TraversalForPrefill (nsIDOMWindow* win, nsVoidArray* wallet_PrefillElement_list, nsString& urlName);
static void wallet_ReleasePrefillElementList(nsVoidArray * wallet_PrefillElement_list);
static void wallet_Initialize(PRBool unlockDatabase=PR_TRUE);
static void wallet_InitListFromAppleAddressBook(nsVoidArray** inList);
...
// EncryptString
//
// don't do any encryption, we're not writing anything to disk.
...
//
// wallet_InitListFromAppleAddressBook
//
// Fills the global |wallet_SchemaToValue_list| with the name/value pairs imported from the
// "me" card of the Apple Address Book. 
//
static void wallet_InitListFromAppleAddressBook(nsVoidArray** inList)
{
  if (!inList)
  ...
  // personal
  wallet_WriteToList("name.last", [[me valueForProperty:kABLastNameProperty] UTF8String], dummy, *inList, PR_FALSE);
  wallet_WriteToList("name.first", [[me valueForProperty:kABFirstNameProperty] UTF8String], dummy, *inList, PR_FALSE);
  wallet_WriteToList("business.companyname", [[me valueForProperty:kABOrganizationProperty] UTF8String], dummy, *inList, PR_FALSE);
  wallet_WriteToList("business.jobtitle", [[me valueForProperty:kABJobTitleProperty] UTF8String], dummy, *inList, PR_FALSE);

  // home phone
  ABMultiValue* phones = [me valueForProperty:kABPhoneProperty];
  NSString* primaryPhone = [phones valueAtIndex:[phones indexForIdentifier:[phones primaryIdentifier]]];
  PhoneNumber* parsedPhone = [[[PhoneNumber alloc] initWithPhoneNumberString:primaryPhone] autorelease];
  wallet_WriteToList("home.phone.loccode", [[parsedPhone areaCodeString] UTF8String], dummy, *inList, PR_FALSE);
  wallet_WriteToList("home.phone.number.prefix", [[parsedPhone prefixString] UTF8String], dummy, *inList, PR_FALSE);
  wallet_WriteToList("home.phone.number.suffix", [[parsedPhone suffixString] UTF8String], dummy, *inList, PR_FALSE);

  // address
  ABMultiValue* address = [me valueForProperty:kABAddressProperty];
  NSDictionary* primaryAddr = [address valueAtIndex:[address indexForIdentifier:[address primaryIdentifier]]];
  wallet_WriteToList("home.street.line1", [[primaryAddr valueForKey:kABAddressStreetKey] UTF8String], dummy, *inList, PR_FALSE);
  wallet_WriteToList("home.city", [[primaryAddr valueForKey:kABAddressCityKey] UTF8String], dummy, *inList, PR_FALSE);
  wallet_WriteToList("home.state", [[primaryAddr valueForKey:kABAddressStateKey] UTF8String], dummy, *inList, PR_FALSE);
  wallet_WriteToList("home.postalcode.prefix", [[primaryAddr valueForKey:kABAddressZIPKey] UTF8String], dummy, *inList, PR_FALSE);
...
//   printf("****** end profile\n");
  ...
  if (!wallet_ValuesReadIn) {
    ...
    wallet_InitListFromAppleAddressBook(&wallet_SchemaToValue_list);
...
{
  // we never want to preview, so |quick| needs to always be true.
  PRBool quick = PR_TRUE;
  ...
  // do all the hard work and fill in |wallet_PrefillElement_list| with the results
  // of the form filling.
  ...
  if (LIST_COUNT(wallet_PrefillElement_list) == 0) {
    if (!gEncryptionFailure) {
      //XXXPINK 
      // Beep? Put something in the status message??
    }
  ...
  // determine if url is on list of urls that should not be previewed. We've modified this to
  // never preview.
  PRBool noPreview = PR_TRUE;
#if UNUSED
  if (!quick) {
    wallet_InitializeURLList();
    nsVoidArray* dummy;
  ...
  /* determine if preview is necessary (which it won't be) */
  if (noPreview || quick) {
 ...
 */
static void
wallet_TraversalForPrefill (nsIDOMWindow* win, nsVoidArray* wallet_PrefillElement_list, nsString& urlName) 
{
  nsresult result;
  if (nsnull != win) {
