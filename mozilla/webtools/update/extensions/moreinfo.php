    ...
    ?>
		...
		<div class="key-point install-box"><div class="install"><?php 
            ...
            if ($appname=="Thunderbird") { 
              ...
              echo "<a href=\"$downloadURL\" onclick=\"return install$rndNum(event,'".addslashes($name)." $version for Thunderbird', '../images/default.png');\"  title=\"Right-Click to Download $name $version\">";
            } else {
                echo"<b><a href=\"$uri\" onclick=\"return install$rndNum(event,'".addslashes($name)." $version', '../images/default.png');\" TITLE=\"Install $name $version (Right-Click to Download)\">";
            }
    ...
    $sql = "SELECT TV.vID, TV.Version, TV.MinAppVer, TV.MaxAppVer, TV.Size, TV.URI, TV.Notes, TV.DateAdded AS VerDateAdded, TA.AppName, TOS.OSName
     ...
     $sql_result = mysql_query($sql, $connection) or trigger_error("MySQL Error ".mysql_errno().": ".mysql_error()."", E_USER_NOTICE);
        while ($row = mysql_fetch_array($sql_result)) {
            ...
            if ($appname=="Thunderbird") {
              $downloadURL=mozupd_buildDownloadURL($uri,$name,$version);
              echo "<a href=\"$downloadURL\" onclick=\"return install$rndNum(event,'".addslashes($name)." $version for Thunderbird', '../images/default.png');\">";
            } else {
                echo"<a href=\"$uri\" onclick=\"return install$rndNum(event,'".addslashes($name)." $version', '../images/default.png');\">";
            }
