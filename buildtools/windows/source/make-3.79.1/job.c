...
#ifdef WINDOWS32
#ifdef MOZILLA
char *default_shell = "shmsdos.exe";
...
#ifdef MOZILLA
  /* Mozilla shmsdos support */
  static char shmsdos_chars[] = "#;\"*?[]&|<>(){}$`^";
  static char *shmsdos_cmds[] = { "cd", "eval", "exec", "exit", "login",
		     "logout", "set", "umask", "wait", "while", "for",
		     "case", "if", ":", ".", "break", "continue",
		     "export", "read", "readonly", "shift", "times",
...
#ifdef MOZILLA
  sh_cmds = shmsdos_cmds;
  sh_chars = shmsdos_chars;
#else /* MOZILLA */
