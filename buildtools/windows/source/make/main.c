...
#undef	FATAL_SIG
   ...
   *
   * Windows NT seems to allow both '/' and '\' as directory separator.
   * Moreover, an absolute pathname on Windows does not always start with
   * a slash or backslash.   It may start with a drive letter and a colon,
   * followed by a slash or a backslash.  We use the rule that
   * if the pathname contains a colon, then it is an absolute path.
   */

  if (current_directory[0] != '\0'
      && argv[0] != 0 && argv[0][0] != '/' && argv[0][0] != '\\'
      && index (argv[0], '/') != 0 && index (argv[0], '\\') != 0
      && index (argv[0], ':') == 0)
    argv[0] = concat (current_directory, "/", argv[0]);
...
#if defined(NETSCAPE)
  /*
   * Before setting the "MAKE" variable, change backslashes (directory 
   * separators on Windows) to forward slashes, otherwise GNU Make will
	...
	if (makefiles->list[i][0] == '-' && makefiles->list[i][1] == '\0')
	  {
		...
		if (n > 0 && fwrite (buf, 1, n, outfile) != (size_t)n)
...
#ifdef NETSCAPE
/*
 * When you do a release build, you can define the macros
 * DEFAULT_SHELL and BUILD_NUMBER to make the following message
 * appear in the version information.
 * 
...
#define BUILD_NUMBER 19980501
  ...
  printf ("\
%sThis version was ported to Win32 by Netscape Communications Corp.\n\
%sDefault shell: %s.  Build number: %d.\n\n", precede, precede,
DEFAULT_SHELL, BUILD_NUMBER);

#endif /* DEFAULT_SHELL, BUILD_NUMBER */
#endif /* NETSCAPE */
