     ...
     int block, err;
{
  ...
  while ((children != 0 || shell_function_pid != 0) &&
	...
	if ((int)c->remote == remote && (int)c->pid == pid)
	  break;
 ...
 */
int ns_spawn(char **argv, char **envp, int *exitCode)
{
    char *p;
    char *cmdLine;
    char *envBlock;
    ...
    /*
     * Before invoking the command, change forward slashes
     * in argv[0] to backslashes.
     */
    for (p = argv[0]; *p; p++) {
        if (*p == '/') {
            *p = '\\';
...
#else	/* MSDOS.  */
#ifdef NETSCAPE
  if (ns_spawn (argv, child->environment, &dos_status) == -1)
    {
      char errMsg[128];
      FormatMessage (
